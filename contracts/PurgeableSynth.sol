...
contract PurgeableSynth is Synth {
    ...
    // The maximum allowed amount of tokenSupply in equivalent xUSD value for this synth to permit purging
    ...
    /**
     * @notice Function that allows owner to exchange any number of holders back to xUSD (for frozen or deprecated synths)
     * @param addresses The list of holders to purge
     */
    function purge(address[] addresses) external optionalProxy_onlyOwner {
        IExchangeRates exRates = exchangeRates();

        uint maxSupplyToPurge = exRates.effectiveValue("xUSD", maxSupplyToPurgeInUSD, currencyKey);
