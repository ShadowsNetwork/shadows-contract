...
schedules to shadows funds sold at various discounts in the token
sale. ShadowsEscrow gives users the ability to inspect their
...
for all DOWS inside the escrow contract, and then allowing
the contract itself to subdivide that pool up proportionally within
itself. Every time the fee period rolls over in the main Shadows
contract, the ShadowsEscrow fee pool is remitted back into the
...
/**
 * @title A contract to hold escrowed DOWS and free them at given schedules.
 */
contract ShadowsEscrow is Owned, LimitedSetup(8 weeks) {
    using SafeMath for uint;

    /* The corresponding Shadows contract. */
    ...
    /* An account's total vested shadows balance to save recomputing this for fee extraction purposes. */
    mapping(address => uint) public totalVestedAccountBalance;

    /* The total remaining vested balance, for verifying the actual shadows balance of this contract against. */
    ...
    constructor(address _owner, IShadows _shadows) public Owned(_owner) {
        shadows = _shadows;
    }

    /* ========== SETTERS ========== */

    function setShadows(IShadows _shadows) external onlyOwner {
        shadows = _shadows;
        emit ShadowsUpdated(_shadows);
    }
    ...
    /**
     * @notice Withdraws a quantity of DOWS back to the shadows contract.
     * @dev This may only be called by the owner during the contract's setup period.
     */
    function withdrawShadows(uint quantity) external onlyOwner onlyDuringSetup {
        shadows.transfer(shadows, quantity);
    }
     ...
     * in this contract, or a corresponding call to shadows.endow(), to ensure that when
     ...
     * @param quantity The quantity of DOWS that will vest.
     */
    function appendVestingEntry(address account, uint time, uint quantity) public onlyOwner onlyDuringSetup {
        ...
        require(
            totalVestedBalance <= shadows.balanceOf(this),
    ...
    /**
     * @notice Construct a vesting schedule to release a quantities of DOWS
    ...
    event ShadowsUpdated(address newShadows);
