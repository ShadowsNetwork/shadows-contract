...
This contract was adapted for use by the Shadows project from the
...
contract ShadowsAirdropper is Owned {
    /* ========== CONSTRUCTOR ========== */
    ...
    function multisend(address _tokenAddress, address[] _destinations, uint256[] _values) external onlyOwner {
        ...
        while (i < _destinations.length) {
            IShadows(_tokenAddress).transfer(_destinations[i], _values[i]);
            i += 1;
        }
    }
