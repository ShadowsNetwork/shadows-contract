...
i.e. The xETH arb pool is assigned 5% of the current Inflationary
supply so it is allocated 72K of the tokens. If that is the only
distribution added then 72K DOWS is deducted from the weeks
inflationary supply and sent to the xETH Arb Pool then the
remainder is sent to the RewardsEscrow Contract for the DOWS
Staking Rewards.
...
contract RewardsDistribution is Owned {
    ...
    /**
     * @notice Address of the Shadows ProxyERC20
     */
    ...
    /**
     * @dev _authority maybe the underlying shadows contract.
     * Remember to set the autority on a shadows upgrade
     */
    constructor(address _owner, address _authority, address _shadowsProxy, address _rewardEscrow, address _feePoolProxy)
        public
        Owned(_owner)
    {
    ...
    function setShadowsProxy(address _shadowsProxy) external onlyOwner {
        shadowsProxy = _shadowsProxy;
    }
    ...
    function distributeRewards(uint amount) external returns (bool) {
        ...
        require(shadowsProxy != address(0), "ShadowsProxy is not set");
        require(feePoolProxy != address(0), "FeePoolProxy is not set");
        require(amount > 0, "Nothing to distribute");
        require(
            IERC20(shadowsProxy).balanceOf(this) >= amount,
        ...
        for (uint i = 0; i < distributions.length; i++) {
            if (distributions[i].destination != address(0) || distributions[i].amount != 0) {
                ...
                // Transfer the DOWS
                IERC20(shadowsProxy).transfer(distributions[i].destination, distributions[i].amount);

                // If the contract implements RewardsDistributionRecipient.sol, inform it how many DOWS its received.
        ...
        IERC20(shadowsProxy).transfer(rewardEscrow, remainder);
