...
contract FeePoolState is SelfDestructible, LimitedSetup {
     ...
     * @param debtEntryIndex The index in the global debt ledger. shadows.shadowsState().issuanceData(account)
     * @param currentPeriodStartDebtIndex The startingDebtIndex of the current fee period
     * @dev onlyFeePool to call me on shadows.issue() & shadows.burn() calls to store the locked DOWS
    ...
    /**
     * @notice Import issuer data from shadowsState.issuerData on FeePeriodClose() block #
