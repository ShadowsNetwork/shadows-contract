...
contract EtherCollateral is Owned, Pausable, ReentrancyGuard, MixinResolver {
    ...
    // Maximum amount of xETH that can be issued by the EtherCollateral contract. Default 5000
    uint256 public issueLimit = SafeDecimalMath.unit() * 5000;

    // Minimum amount of ETH to create loan preventing griefing and gas consumption. Min 1ETH = 0.6666666667 xETH
    ...
    function openLoan() external payable notPaused nonReentrant returns (uint256 loanID) {
        ...
        // Require xETH to mint does not exceed cap
        ...
        // Issue the synth
        synthxETH().issue(msg.sender, loanAmount);
    ...
    function _closeLoan(address account, uint256 loanID) private {
        ...
        require(
            synthxETH().balanceOf(msg.sender) >= synthLoan.loanAmount,
        ...
        synthxETH().burn(account, synthLoan.loanAmount);

        // Fee Distribution. Purchase xUSD with ETH from Depot
        require(synthxUSD().balanceOf(depot()) >= totalFees, "The xUSD Depot does not have enough xUSD to buy for fees");
        depot().exchangeEtherForSynths.value(totalFees)();

        // Transfer the xUSD to distribute to DOWS holders.
        synthxUSD().transfer(FEE_ADDRESS, synthxUSD().balanceOf(this));
    ...
    function synthxETH() internal view returns (ISynth) {
        return ISynth(resolver.requireAndGetAddress("SynthxETH", "Missing SynthxETH address"));
    }
