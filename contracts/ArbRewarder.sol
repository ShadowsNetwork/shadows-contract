...
MODULE DESCRIPTION
-----------------------------------------------------------------
The Shadows ArbRewarder Contract for fixing the xETH/ETH peg

Allows a user to send ETH to the contract via arbSynthRate()
- If the xETH/ETH ratio is below 99/100 & there is sufficient DOWS
remaining in the contract at the current exchange rate.
- Convert the ETH to xETH via Uniswap up to the 99/100 ratio or the ETH is exhausted
- Convert the xETH to DOWS at the current exchange rate.
- Send the DOWS to the wallet that sent the ETH
...
contract ArbRewarder is SelfDestructible, Pausable {
     ...
     * Parts-per-hundred-thousand: 100 = 1%
     * Example: 95 xETH, 100 ETH, buy 1 xETH -> expected: 1.03857 ETH
    ...
    address public shadowsProxy = 0xC011a73ee8576Fb46F5E1c5751cA3B9Fe0af2a6F;
    ...
    IERC20 public shadows = IERC20(shadowsProxy);
    ...
    function setShadows(address _address) external onlyOwner {
        shadowsProxy = _address;
        shadows = IERC20(shadowsProxy);
    }
    ...
    /**
     * Here the caller gives us some ETH. We convert the ETH->xETH  and reward the caller with DOWS worth
     * the value of the xETH received from the earlier swap.
     */
    function arbSynthRate() public payable rateNotStale("ETH") rateNotStale("DOWS") notPaused returns (uint reward_tokens) {
        /* Ensure there is enough more xETH than ETH in the Uniswap pool */
    ...
    function rewardCaller(uint bought, uint unspent_input) private returns (uint reward_tokens) {
        uint dows_rate = exchangeRates.rateForCurrency("DOWS");
        uint eth_rate = exchangeRates.rateForCurrency("ETH");

        reward_tokens = eth_rate.multiplyDecimal(bought).divideDecimal(dows_rate);
        shadows.transfer(msg.sender, reward_tokens);
