...
MODULE DESCRIPTION
-----------------------------------------------------------------
Escrows the DOWS rewards from the inflationary supply awarded to
users for staking their DOWS and maintaining the c-ratio target.
...
contract RewardEscrow is Owned {
    ...
    * There are 5 years of the supply schedule */
    ...
    constructor(address _owner, IShadows _shadows, IFeePool _feePool) public Owned(_owner) {
        shadows = _shadows;
        feePool = _feePool;
    }
    ...
    /**
     * @notice set the shadows contract address as we need to transfer DOWS when the user vests
     */
    ...
    function appendVestingEntry(address account, uint quantity) public onlyFeePool {
        ...
        require(
            totalEscrowedBalance <= shadows.balanceOf(this),
