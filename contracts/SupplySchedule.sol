...
The DOWS supply schedule contract determines the amount of DOWS tokens
mintable over the course of 195 weeks.

Exponential Decay Inflation Schedule

Shadows.mint() function is used to mint the inflationary supply.

The mechanics for Inflation Smoothing and Terminal Inflation 
have been defined in these sips
https://sips.shadows.io/sips/sip-23
https://sips.shadows.io/sips/sip-24

The previous DOWS Inflation Supply Schedule is at 
...
contract SupplySchedule is Owned {
    ...
    // The number of DOWS rewarded to the caller of Shadows.mint()
    ...
    // Address of the ShadowsProxy for the onlyShadows modifier
    address public shadowsProxy;

    // Max DOWS rewards for minter
    ...
    function mintableSupply() external view returns (uint) {
        ...
        while (remainingWeeksToMint > 0) {
            ...
            if (currentWeek < SUPPLY_DECAY_START) {
                ...
                // Terminal supply is calculated on the total supply of Shadows including any new supply
                // We can compound the remaining week's supply at the fixed terminal rate
                uint totalSupply = IShadows(shadowsProxy).totalSupply();
    ...
    */
    ...
    /**
     * @notice Record the mint event from Shadows by incrementing the inflation 
     * week counter for the number of weeks minted (probabaly always 1)
     * and store the time of the event.
     * @param supplyMinted the amount of DOWS the total supply was inflated by.
     * */
    function recordMintEvent(uint supplyMinted) external onlyShadows returns (bool) {
    ...
    /**
     * @notice Sets the reward amount of DOWS for the caller of the public 
     * function Shadows.mint(). 
     * This incentivises anyone to mint the inflationary supply and the mintr 
     * Reward will be deducted from the inflationary supply and sent to the caller.
     * @param amount the amount of DOWS to reward the minter.
     * */
    ...
    /**
     * @notice Set the ShadowsProxy should it ever change.
     * SupplySchedule requires Shadows address as it has the authority
     * to record mint event.
     * */
    function setShadowsProxy(IShadows _shadowsProxy) external onlyOwner {
        require(_shadowsProxy != address(0), "Address cannot be 0");
        shadowsProxy = _shadowsProxy;
        emit ShadowsProxyUpdated(shadowsProxy);
    }

    // ========== MODIFIERS ==========

    /**
     * @notice Only the Shadows contract is authorised to call this function
     * */
    modifier onlyShadows() {
        require(
            msg.sender == address(Proxy(shadowsProxy).target()),
            "Only the shadows contract can perform this action"
        );
        _;
    }
    ...
    /**
     * @notice Emitted when the DOWS minter reward amount is updated
     * */
    event MinterRewardUpdated(uint newRewardAmount);

    /**
     * @notice Emitted when setShadowsProxy is called changing the Shadows Proxy address
     * */
    event ShadowsProxyUpdated(address newAddress);
}
