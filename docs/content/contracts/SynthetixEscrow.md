...
This contract holds the DOWS which were escrowed at the time of the original token sale, releasing them according to a defined schedule.
...
**Source:** [ShadowsEscrow.sol](https://github.com/Shadowsio/shadows/blob/master/contracts/ShadowsEscrow.sol)
...
<centered-image>
    ![ShadowsEscrow inheritance graph](../img/graphs/ShadowsEscrow.svg)
</centered-image>
...
Initialises the [`Shadows`](Shadows.md) contract address, and the inherited [`Owned`](Owned.md) instance.

??? example "Details"

    **Signature**

    `constructor(address _owner, Shadows _shadows) public`
...
Transfers a quantity of DOWS back to the Shadows contract.

This was callable by the owner during the setup period in case too much DOWS was deposited into the escrow contract.

??? example "Details"

    **Signature**

    `withdrawShadows(uint quantity) external`
