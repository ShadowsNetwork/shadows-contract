...
This is the mechanism for distributing DOWS rewards from the inflationary supply. When an DOWS staker claims fees, the inflationary reward component is escrowed in this contract and an entry is added to an escrow schedule for that staker for them to claim after a year. These vesting schedules can only be appended to by the [FeePool](FeePool.md) contract.

The logic of RewardEscrow is derived from the [ShadowsEscrow](ShadowsEscrow.md) contract.

**Source:** [RewardEscrow.sol](https://github.com/Shadowsio/shadows/blob/master/contracts/RewardEscrow.sol)
...
The address of the main [`Shadows`](Shadows.md) contract.
...
A record of the total remaining vested balance in this contract, which should be equal to the actual DOWS balance.
...
Initialises the [`Shadows`](Shadows.md) and [`FeePool`](FeePool.md) contract addresses, and the inherited [`Owned`](Owned.md) instance.

??? example "Details"

    **Signature**

    `constructor(address _owner, Shadows _shadows, FeePool _feePool) public`
...
Sets the address of the [`Shadows`](Shadows.md) contract, so that escrowed DOWS can be transferred to accounts claiming them.
...
Returns a particular schedule entry for an account, which is a pair of uints: `(vesting timestamp, DOWS quantity)`.
...
Returns the quantity of DOWS a given schedule entry will yield.
...
Returns the DOWS quantity of the next vesting entry. Returns `0` if there is no such entry.
...
??? example "Details"
    ...
    * The balance of DOWS in the escrow contract must be sufficient to supply the new vesting entry.
...
Records that the DOWS contract address was altered.
