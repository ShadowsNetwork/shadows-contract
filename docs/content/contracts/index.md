...
Here you will find descriptions of the smart contract interfaces of every smart contract in the Shadows system. These documents go a bit further than the code does. These documents try to emphasise the reasons behind the architecture, specifically highlighting interactions between system components. The technical aspects of the system are also discussed together with the details of the incentive mechanism, and with links back to governance processes.

Developers wishing to understand Shadows code and the tradeoffs within it will be well-advised to read these documents alongside the Solidity itself.
...
The Shadows system is composed of a number of core contract complexes, and several auxiliary contracts that enhance token liquidity and general system functionality. These on-chain entities and how they communicate is reflected in the following high-level diagram.

<centered-image>
    ![Contract architecture graph](../img/graphs/contract-architecture.svg)
</centered-image>

The oracle, Shadows, synths, inflationary supply, and fee pool elements comprise the core of the system. The arbitrage and depot contracts are supporting components which, while they do not alter the fundamental behaviour of system tokens, ensure the economic health of the system. They do this by making sure that DOWS and synths flow freely through markets like Uniswap and Mintr, improving liquidity and encouraging prices to be close to their theoretically correct values.
...
Shadows contract communicates with [synths](#synths) to manage their supply, as well as the [fee pool](#fee-pool) to remit fees when synth exchanges occur. In order to properly convert between synths, and to understand value of debt induced by minting tokens, the Shadows contract retrieves current token prices from the [oracle](#oracle). This contract also communicates with the [inflationary supply](#inflationary-supply) complex to mint the correct quantity when expanding the supply of DOWS, and to distribute the new tokens appropriately.

Along with the debt ledger, which is a time series history of the overall value of the shadows ecosystem, issuance data for the current fee period is updated whenever synths are issued or burnt. This complex is also responsible for pushing historical issuance information to the fee pool, so that as fee periods roll over, a record of what portion of the total system debt is owned by different issuers is known when computing their fee entitlements.

**Constituent Contracts**

| Contract                              | Description                                                                                                      |
| ------------------------------------- | ---------------------------------------------------------------------------------------------------------------- |
| [`Shadows`](Shadows.md)           | The main token contract.                                                                                         |
| [`ShadowsState`](ShadowsState.md) | An auxiliary state contract that sits alongside Shadows, which tracks current issuer data and the debt ledger. |
...
Synths implement their own issuance and burning logic, but only the [Shadows](#shadows) and [fee pool](#fee-pool) contracts may invoke them. The Shadows contract uses these functions for issuance and burning by stakers, while the fee pool uses them to burn fees at the fee address and issue them to the claimers address.
...
!!! example "Responsibilities"
    ...
    _ Accumulates synth exchange fees, holding them as a pool of xUSD.
    _ Directs the [`RewardEscrow`](RewardEscrow.md) to escrow inflationary DOWS rewards for eligible issuers.
...
Since the collection of exchange fees on synths is mediated through the [`Shadows.exchange`](Shadows.md#exchange) function, the fee pool interacts closely with both the [`Shadows`](Shadows.md) and [`Synth`](Synth.md) contracts.

The [`Shadows`](Shadows.md) contract informs the fee pool when [fees are collected](FeePool.md#feepaid), and it is allowed to append historic issuance records to its own [account issuance ledger](FeePoolState.md#accountissuanceledger). The fee pool mostly interacts with other system components through [`Shadows`](Shadows.md). For example, it only interacts with the oracle through the Shadows contract, in order to issue fees and rewards. It also retrieves other data from there, like debt ledger information, issuance and collateralisation ratios, and the addresses of synth contracts.
...
!!! example "Responsibilities"
    ...
    _ Defines the schedule according to which DOWS tokens are generated from the inflationary supply.
...
The inflationary supply complex is concerned with controlling the flow of new DOWS tokens being injected into the market. In this capacity it communicates with the [`Shadows`](Shadows.md) contract. The actual fraction of the weekly DOWS rewards that a particular account is entitled to claim is computed by the [fee pool](#fee-pool), which is able to direct the [`RewardEscrow`](RewardEscrow.md) and [`RewardsDistribution`](RewardsDistribution.md) contracts as to how they should distribute the new tokens.

**Constituent Contracts**

| Contract                                        | Description                                                                                                                                                      |
| ----------------------------------------------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| [`SupplySchedule`](SupplySchedule.md)           | Determines the rate that inflationary DOWS tokens are released.                                                                                                   |
| [`RewardEscrow`](RewardEscrow.md)               | Receives inflationary DOWS rewards to be distributed after a year escrow.                                                                                         |
| [`RewardsDistribution`](RewardsDistribution.md) | Apportions designated quantities of inflationary rewards to the [`RewardEscrow`](RewardEscrow.md) and [`ShadowsAirdropper`](ShadowsAirdropper.md) contracts. |
| [`ShadowsAirdropper`](ShadowsAirdropper.md) | Distributes tokens from the inflationary supply to individual residents of the the UniSwap ETH/xETH liquidity pool.                                              |
| [`ShadowsEscrow`](ShadowsEscrow.md)         | Holds the escrowed balances of DOWS from the original token sale.                                                                                                 |
| [`EscrowChecker`](EscrowChecker.md)             | Augments the [`ShadowsEscrow`](ShadowsEscrow.md) contract with a function for dApps to conveniently query it.                                                |
...
| Contract                            | Description                                                                                                                                                                                                                                                      |
| ----------------------------------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| Oracle                              | The oracle is responsible for collecting and updating all token prices known to the Shadows system. Although it is not a contract, it controls a known Ethereum address from which price updates are sent to the [`ExchangeRates`](ExchangeRates.md) contract. |
| [`ExchangeRates`](ExchangeRates.md) | The Shadows exchange rates contract which receives token prices from the oracle, and supplies them to all contracts that need it.                                                                                                                              |
...
The [`Depot`](Depot.md) is a vendor contract that allows users to exchange their ETH for xUSD or DOWS, or their xUSD for DOWS. It also allows users to deposit Synths to be sold in exchange for ETH.

The depot has its own dedicated oracle, and all exchanges are performed at the current market prices, assuming xUSD is priced at one dollar.
...
The [ArbRewarder](ArbRewarder.md) automates the process of arbitraging the ETH/xETH price on UniSwap through Shadows conversion functions.
...
!!! example "Responsibilities"
    ...
    _ Tracks the latest instances of all contracts required in the Shadows system, allowing them to be queried by a `bytes32` name
...
| [`ExternStateToken`](ExternStateToken.md) | A partial ERC20 token contact with an external state, which all tokens in Shadows are built upon.              |
...
over the history of the Shadows project in a continuing process to
validate the integrity of its smart contract system.

| Auditor                               | Subject                                                                                                                                                                      | Date          | Commit                                                                                            |
| ------------------------------------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ------------- | ------------------------------------------------------------------------------------------------- |
| [iosiro](https://www.iosiro.com/)     | [Shadows Gas Limits](https://www.iosiro.com/audits/shadows-smart-contract-audit-phase-2)                                                                                 | August 2019   | [fdd4782](https://github.com/Shadowsio/shadows/tree/fdd4782ebebd7b4892c8a68000f76708d5d1aa7b) |
| [iosiro](https://www.iosiro.com/)     | [Shadows Multicurrency](https://www.iosiro.com/audits/shadows-smart-contract-audit)                                                                                      | February 2019 |                                                                                                   |
| [Sigma Prime](https://sigmaprime.io/) | [Shadows Multicurrency](https://www.shadows.io/uploads/sigma-prime-shadows-audit-report_2_0.pdf)                                                                       | January 2019  |                                                                                                   |
...
| [Cryptecon](https://cryptecon.org/)   | [Havven nUSD economics](https://old.havven.io/uploads/havven_cryptecon_report_may_2018.pdf) ([blog post](https://blog.shadows.io/havven-validated-by-cryptecon-analysis/)) | June 2018     |                                                                                                   |
