...
**Old:** Depot.sol: Allows users to exchange ETH for xUSD and DOWS (has not yet been updated for multicurrency).

Throughout, the contract assumes that xUSD is always worth exactly US\$1. So: a) this will only work with `xUSD`. b) there's a profit opportunity if the `xUSD` is off its peg.
...
| amount | `uint`    | The quantity of xUSD deposited. |
...
The address of the main [`Shadows`](Shadows.md) contract; the depot contains DOWS.
...
The address of the xUSD [`Synth`](Synth.md), which are the synth held in the depot.
...
The address of the [`FeePool`](FeePool.md) contract. Since transfer fees were eliminated in [SIP-19](https://sips.shadows.io/sips/sip-19), this is not really used anymore. All the fee pool functions this contract calls have now been replaced with effective no-ops.
...
The address where ether and synths raised by selling DOWS are sent.
...
The address which provides the usd prices of DOWS and ether. This is not the same oracle address as in [`ExchangeRates`](ExchangeRates.md#oracle).
...
The last time [`usdToDowsPrice`](#usdtodowsprice) and [`usdToEthPrice`](#usdtoethprice) were updated by the [`oracle`](#oracle) calling [`updatePrices`](#updateprices).
...
Users can deposit xUSD to be sold on the depot. This variable holds the queue of open deposits, which are sold in the order they were deposited.
...
The total quantity of xUSD currently in the [`deposits`](#deposits) queue to be purchased.
...
The minimum xUSD quantity required for a deposit to be added to the queue. Initialised to 50.0.
...
Deposits of less than [`minimumDepositAmount`](#minimumdepositamount) xUSD are not placed on the [`deposits`](#deposits) queue. Instead, they are kept here so that the depositor can withdraw them.
...
??? example "Details"
    ...
    **Signature**
    ...
    `constructor(address _owner, address _fundsWallet, Shadows _shadows, Synth _synth, FeePool _feePool, address _oracle, uint _usdToEthPrice, uint _usdToDowsPrice) public`
...
Allows the owner to set the address of the [`shadows`](#shadows) contract.

??? example "Details"

    **Signature**

    `setShadows(Shadows _shadows)`
...
Allows the oracle address to update the USD [ETH](#usdToEthPrice) and [DOWS](#usdToDowsPrice) prices known to the depot.
...
??? example "Details"
    ...
    **Signature**
    ...
    `updatePrices(uint newEthPrice, uint newShadowsPrice, uint timeSent) external`
    ...
    * [`PricesUpdated(newEthPrice, newShadowsPrice, timeSent)`](#pricesupdated)
...
Sells xUSD to callers who send ether. The synths are sold from the [`deposits`](#deposits) queue in the order they were deposited.
...
### `exchangeEtherForShadows`

- `exchangeEtherForShadows() returns (uint)`: Requires that the contract is not paused, and that the prices are not stale. Converts the received ether to a quantity of DOWS with `shadowsReceivedForEther`. Sends the ether to `fundsWallet`, sends the converted quantity of DOWS to the message sender from the contract's own reserves. Returns the DOWS quantity sent. If the contract has insufficient DOWS, then the transfer will fail and the transaction will revert.

---

### `exchangeEtherForShadowsAtRate`

- `exchangeEtherForShadowsAtRate(uint guaranteedEtherRate, uint guaranteedShadowsRate) returns (uint)`: As `exchangeEtherForSynthsAtRate` is to `exchangeEtherForSynths`, this is to `exchangeEtherForShadows`.

---

### `exchangeSynthsForShadows`

- `exchangeSynthsForShadows(uint synthAmount) returns (uint)`: Identical to `exchangeEtherForShadows`, but perform the price conversion with `shadowsReceivedForSynths`. The amount of synths to send is provided as a function argument, and then transferred to `fundsWallet` with `transferFrom`, so this function requires the caller to have approved the depot contract to make such a withdrawal. Note that this assumes that xUSD is worth exactly one dollar.

---

### `exchangeSynthsForShadowsAtRate`

- `exchangeSynthsForShadowsAtRate(uint synthAmount, uint guaranteedRate) returns (uint)`: As per `exchangeEtherForShadowsAtRate`.

---

### `withdrawShadows`

- `withdrawShadows(uint amount)`: Only callable by the contract owner. Allows the owner to transfer DOWS out of the Depot to themselves.
...
True if [`usdToDowsPrice`](#usdtodowsprice) and [`usdToEthPrice`](#usdtoethprice) are too old to be considered usably up to date.
...
### `shadowsReceivedForSynths`

Computes the quantity of DOWS received in exchange for a given quantity of xUSD at current prices, assuming xUSD are worth \$1. This is equivalent to:

$$
Q_\text{DOWS} = Q_\text{xUSD} \times \frac{1}{\pi_\text{DOWS}}
$$
...
??? example "Details"
    ...
    **Signature**
    ...
    `shadowsReceivedForSynths(uint amount) public view returns (uint)`

---

### `shadowsReceivedForEther`

Computes the quantity of DOWS received in exchange for a given quantity of Ether at current prices. This is equivalent to:

$$
Q_\text{DOWS} = Q_\text{ETH} \times \frac{\pi_\text{ETH}}{\pi_\text{DOWS}}
$$
...
??? example "Details"
    ...
    **Signature**
    ...
    `shadowsReceivedForEther(uint amount) public view returns (uint)`

---

### `synthsReceivedForEther`

Computes the quantity of xUSD received in exchange for a given quantity of ETH at current prices. This is equivalent to:

$$
Q_\text{xUSD} = Q_\text{ETH} \times \pi_\text{DOWS}
$$
...
- `ShadowsUpdated(Shadows newShadowsContract)`
...
- `PricesUpdated(uint newEthPrice, uint newShadowsPrice, uint timeSent)`
