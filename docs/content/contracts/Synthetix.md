...
**Source:** [Shadows.sol](https://github.com/Shadowsio/shadows/blob/master/contracts/Shadows.sol)
...
<centered-image>
    ![Shadows inheritance graph](../img/graphs/Shadows.svg)
</centered-image>
...
??? example "Details"
    ...
    - [`Depot`](Depot.md): The depot trades DOWS and therefore knows the Shadows address. \* [`ArbRewarder`](ArbRewarder.md): The ArbRewarder knows the Shadows address because it exchanges DOWS.
    - [`Exchanger`](Exchanger.md) The helper contract that performs the heavy lifting for `exchange()` and `settle()`.
    - [`ExchangeRates`](ExchangeRates.md): The Shadows contract fetches prices from the exchange rates contract to facilitate synth exchange and to determine the value of various quantities of synths.
    ...
    - [`Proxy`](Proxy.md): The Shadows contract, which is [`Proxyable`](Proxyable.md), exists behind a `CALL`-style proxy for upgradeability.
    - [`RewardEscrow`](RewardEscrow.md): This is similar to the ShadowsEscrow contract, but it is where the DOWS inflationary supply is kept before it is released to Synth issuers.
    ...
    - [`SupplySchedule`](SupplySchedule.md): The supply schedule determines the rate at which DOWS are released from the inflationary supply.
    - [`Synth`](Synth.md): Shadows manages the supply of synths. It keeps track of which ones exist, and they are all issued and burnt from the Shadows contract. The Shadows contract is also responsible for exchange between different synth flavours.
    - [`ShadowsEscrow`](ShadowsEscrow.md): The escrow contract keeps track of DOWS owed to participants in the initial token sale, and releases them according to specified vesting schedules.
...
Returns the total DOWS owned by the given account, locked and unlocked, escrowed and unescrowed. This is the quantity of DOWS synths can be issued against.

This is computed as the sum of [`Shadows.balanceOf(account)`](TokenState.md#balanceof), [`ShadowsEscrow.balanceOf(account)`](ShadowsEscrow.md#balanceof), and [`RewardEscrow.balanceOf(account)`](RewardEscrow.md#balanceof); so an account may issue synths against both its active balance and its unclaimed escrow funds.
...
The ratio between value of synths that an account has issued and the value of the collateral they control. That is, this is just [`debtBalanceOf(issuer, "DOWS") /`](#debtbalanceof) [`collateral(issuer)`](#collateral).

Ideally, issuers should maintain their collateralisation ratio at a level less than the [global issuance ratio](ShadowsState.md#issuanceratio), and they are incentivised to do this by the [fees they can claim](FeePool.md#claim) if they do so.
...
Reports the quantity of a given currency required to free up all DOWS locked in given account.

If $\mathrm{X}$ is the [total value of all issued synths](#totalissuedsynths), and $\check{\omega}$ is fraction of that value currently accounted for by this account's locked DOWS, then the result is simply:
...
Where $\omega$ is the account's debt ownership fraction at the time it [last issued or burnt](ShadowsState.md#issuancedata) synths, which produced the $\Delta_\text{entry}$ item in the [debt ledger](ShadowsState.md#debtledger). $\Delta_\text{last}$ is the latest value on the ledger. This logic is much the same as that found in [`FeePool._effectiveDebtRatioForPeriod`](FeePool.md#_effectivedebtratioforperiod). The actual value of $\omega$ is set in [`_addToDebtRegister`](#_addtodebtregister) and [`_removeFromDebtRegister`](#_removefromdebtregister).
...
The maximum number of a given synth that is issuable against the issuer's collateral. This is simply [`issuanceRatio *`](ShadowsState.md#issuanceratio) [`collateral(issuer)`](#collateral), priced in the requested currency.
...
The remaining xUSD synths this account can issue.
...
The quantity of DOWS this account can transfer given that a portion of it may be locked due to issuance.

If $\text{balance}$ is [`balanceOf(account)`](TokenState.md#balanceof), and $\text{lockedDows}$ is [`debtBalanceOf(account, "DOWS") / ShadowsState.issuanceRatio`](#debtbalanceof), the function returns $max(0, \text{balance} - \text{lockedDows})$. Escrowed tokens are not taken into account in this computation, so unescrowed tokens are locked immediately.

???+ info "A Note on Price Motion"

    The value of $\text{lockedDows}$ depends on the current ($\pi$) and previous ($\pi'$) prices being reported by the oracle, and the issuance ratio ($\rho$).
    ...
    In such a situation, we can think of each user having issued a particular quantity of synths. This quantity depends on the prices of synths and DOWS at the time of issuance.

    $$
    Q_s = \rho \ \frac{\pi'_c}{\pi'_s} \ Q_c
    $$

    Whose value at the present time priced [in terms of DOWS](#effectivevalue), which is what [`debtBalanceOf(account, "DOWS")`](#debtbalanceof) returns, is:

    $$
    {V_s}^{c} = \rho \ \frac{\pi'_c}{\pi'_s} \ \pi_c \ Q_c
    $$

    Note that this computation has a factor of $\rho$ in it, and this must be divided out in order to ascertain the quantity of DOWS which are presently locked.

    $$
    \text{lockedDows} = \frac{{V_s}^{c}}{\rho} = \frac{\pi'_c}{\pi'_s} \ \pi_c \ Q_c
    $$

    Which is to say that the quantity of DOWS locked in this situation depends on the price.
...
??? example "Details"
    ...
    **Signature**
    ...
    `transferableShadows(address account) public view returns (uint)`

    **Modifiers**

    * [`rateNotStale("DOWS")`](#ratenotstale)
...
[Issues](Synth.md#issue) a new quantity of `xUSD` into the calling address. The new debt issuance is recorded with [`_addToDebtRegister`](#_addtodebtregister), and the account's issuance records are updated with [`_appendAccountIssuanceRecord`](#_appendaccountissuancerecord).
...
Issues the [maximum quantity](#remainingissuablesynths) `xUSD` issuable by the caller of a particular synth flavour. Otherwise, this operates exactly as [`issueSynths`](#issuesynths) does.
...
This function is responsible for creating the inflationary DOWS supply. It is a public function, so any address can ensure new tokens are released on schedule. When a new quantity is minted, the calling address is rewarded with a small incentive of DOWS tokens, defined by [`SupplySchedule.minterReward`](SupplySchedule.md#minterreward).

The supply is released according to the schedule defined in [`SupplySchedule.schedules`](SupplySchedule.md#schedules), being sent to the [`RewardsDistribution`](RewardsDistribution.md#distributerewards) contract for distribution and escrow. The total supply DOWS supply is thus increased by the quantity specified by the schedule.
...
??? example "Details"
    ...
    * [`Transfer(shadows, rewardDistribution, newSupply - minterReward)`](ExternStateToken.md#transfer)
    * [`Transfer(shadows, msg.sender, minterReward)`](ExternStateToken.md#transfer)
...
A successful transfer requires the message sender to have sufficient balance, accounting for [locked DOWS](#transferableshadows).
...
??? example "Details"
    ...
    * `value` must not exceed [`transferableShadows(messageSender)`](#transferableshadows)
...
A successful transfer requires the token owner to have sufficient balance, accounting for [locked DOWS](#transferableshadows).
...
??? example "Details"
    ...
    * `value` must not exceed [`transferableShadows(from)`](#transferableshadows)
...
??? example "Details"
    ...
    * The xUSD synth cannot be removed.
...
This is only used by [`PurgeableSynth.purge`](#PurgeableSynth.md#purge) in order to convert outstanding synths into xUSD. No exchange fee is charged on such liquidations.
...
Conversion is performed by burning the specified quantity of the source currency from the `from` address, and issuing an [equivalent value](#effectivevalue) of the destination currency into the destination address, minus a [fee](FeePool.md#amountreceivedfromexchange) if `chargeFee` is true. This fee is issued into the [fee address](FeePool.md#feeaddress) in xUSD, and the fee pool is [notified](FeePool.md#feepaid).
...
??? example "Details"
    ...
    * The destination address must not be the Shadows contract itself.
    * The destination address must not be the Shadows proxy.
...
Whenever synths are issued, this function is invoked to update the [debt ledger](ShadowsState.md#debtledger). It computes the factor the issuance changes the overall supply by and appends the resulting entry to the debt ledger. This entry is saved as a [27-decimal fixed point number](SafeDecimalMath.md).

In addition, the caller's [current issuance data](ShadowsState.md#setcurrentissuancedata) is updated and, if they haven't issued before, the [total issuer count is incremented](ShadowsState.md#incrementtotalissuercount).
...
???+ info "Debt Ledger and Issuance Data"
    ...
    **Definitions**
    ...
    | ---------------- | ---------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
    | $\Delta$         | See the **Ledger Updates** section below.            | The [debt ledger](ShadowsState.md#debtledger): an array of debt movement factors, indicating the size of the issued system debt over time. $\Delta_n$ is the $n^{th}$ entry in the ledger.                                                                                                                                                |
    | $X$              | $\frac{1}{\pi_\text{xUSD}}\sum_{c}{\pi_c \sigma_c}$  | The xUSD value of all issued synths ([`totalIssuedSynths`](#totalissuedsynths)) at current prices.                                                                                                                                                                                                                                          |
    | $\widehat{\chi}$ | $\omega \frac{\Delta_\text{last}}{\Delta_{entry}} X$ | The xUSD value of the account's existing issuance debt at current prices ([`debtBalanceOf`](#debtbalanceof)). $\omega$ is the calling account's last recorded owership fraction of the total system debt. We will also refer to the adjusted current ownership fraction $\check{\omega} = \omega \frac{\Delta_\text{last}}{\Delta_{entry}}$. |
    | $\chi$           |                                                      | The xUSD value of the newly-issued synth debt; the new total debt will be $X + \chi$.                                                                                                                                                                                                                                                        |
...
Whenever synths are burnt, this function is invoked to update the [debt ledger](ShadowsState.md#debtledger). It computes the factor the burning changes the overall supply by and appends the resulting entry to the debt ledger. This entry is saved as a [27-decimal fixed point number](SafeDecimalMath.md).

In addition, the caller's [current issuance data](ShadowsState.md#setcurrentissuancedata) is updated and, if they are burning all their tokens, the [total issuer count is decremented](ShadowsState.md#decrementtotalissuercount).
