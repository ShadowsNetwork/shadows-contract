...
**Source:** [FeePool.sol](https://github.com/Shadowsio/shadows/blob/master/contracts/FeePool.sol)
...
??? example "Details"
    ...
    - [`Shadows`](Shadows.md): The fee pool needs the Shadows address for a onlyShadows modifer for storing a minters issue and burn events to track their debt % of the system.
    - [`ShadowsState`](ShadowsState.md): The fee pool retrieves the global issuance ratio, and queries the debt ledger directly from the Shadows state contract.
    - [`Synth`](Synth.md): The fee pool, retrieving their addresses from the Shadows contract, directly burns and issues xUSD when transferring fees. Fees are denominated and paid out in xUSD. Synths themselves do not know the fee pool address directly, but ask the fee pool's proxy for its target.
    ...
    - [`RewardEscrow`](RewardEscrow.md): The contract into which inflationary DOWS rewards are paid by the fee pool so that they can be escrowed for a year after being claimed.
    ...
    - [`Depot`](Depot.md): Allows users to exchange between Synths, DOWS, and Ether. The Depot uses the fee pool to know what transfer fees were being incurred on its transfers, although the transfer fee has been nil since before [SIP-19](https://sips.shadows.io/sips/sip-19).
...
| startingDebtIndex   | `uint`                                     | The length of [`ShadowsState.debtLedger`](ShadowsState.md#debtledger) at the time this fee period began.                                                                                                                                                                                                                      |
| startTime           | `uint`                                     | The current timestamp when this fee period began.                                                                                                                                                                                                                                                                                 |
| feesToDistribute    | `uint` ([18 decimals](SafeDecimalMath.md)) | The total of fees to be distributed in this period, in xUSD. This increases when fees are collected in the current period or when unclaimed fees roll over from the oldest period to the second oldest. See [`feePaid`](#feepaid) and [`closeCurrentPeriod`](#closecurrentperiod).                                                |
| feesClaimed         | `uint` ([18 decimals](SafeDecimalMath.md)) | The number of fees that have already been claimed during this period.                                                                                                                                                                                                                                                             |
| rewardsToDistribute | `uint` ([18 decimals](SafeDecimalMath.md)) | The total of inflationary rewards to be distributed in this period, in DOWS. This increases when new rewards are minted by [`Shadows.mint`](Shadows.md#mint)/[`rewardsMinted`](#rewardsminted), or when unclaimed rewards roll over from the oldest period to the second oldest ([`closeCurrentPeriod`](#closecurrentperiod)). |
...
The address where fees are pooled as xUSD.
...
This was reduced from 6 to 3 as part of [SIP-4](https://sips.shadows.io/sips/sip-4), but note the inconsistency with the corresponding constant in [`FeePoolState`](FeePoolState.md#fee_period_length), which cannot be altered.
...
A threshold that allows issuers to be undercollateralised by up to 10%. Users may claim fees if their [collateralisation ratio](Shadows.md#collateralisationratio) is below the target [issuance ratio](ShadowsState.md#issuanceratio) plus 10%.
...
The main [`Shadows`](Shadows.md) contract.
...
The associated [`ShadowsState`](ShadowsState.md) contract.

**Type:** `ShadowsState public`
...
??? example "Details"
    ...
    **Signature**
    ...
    `constructor(address _proxy, address _owner, Shadows _shadows, FeePoolState _feePoolState, FeePoolEternalStorage _feePoolEternalStorage, IShadowsState _shadowsState, IShadowsEscrow _rewardEscrow, address _rewardsAuthority, uint _exchangeFeeRate) public`
...
As of [SIP-19](https://sips.shadows.io/sips/sip-19), this is just the identity function, since there are no longer any transfer fees. It is only used by the [`Depot`](Depot.md) contract.
...
Computes the quantity received if a quantity of Synths is exchanged into another flavour. The amount received is the quantity sent minus the [exchange fee](#exchangefeeincurred), as per the logic in [`Shadows._internalExchange`](Shadows.md#_internalexchange).
...
Return the total of fees and rewards available to be withdrawn by this account. The result is reported as a `[fees, rewards]` pair denominated in the requested Synth flavour and DOWS, respectively.
...
Returns an array of [`FEE_PERIOD_LENGTH`](#fee_period_length) `[fees, rewards]` pairs owed to an account for each [recent fee period](#recentfeeperiods) (including the current one). Fees are denominated in xUSD and rewards in DOWS.
...
A account is able to claim fees if its [collateralisation ratio](Shadows.md#collateralisationratio) is less than 110% of the [global issuance ratio](ShadowsState.md#issuanceratio).
...
Returns the collateralisation level a user can reach before they cannot claim fees. This is simply [`ShadowsState.issuanceRatio *`](ShadowsState.md#issuanceratio) [`(1 + TARGET_THRESHOLD)`](#target_threshold). The result is returned as a [18-decimal fixed point number](SafeDecimalMath.md).
...
Computes the total DOWS rewards available to be withdrawn. This simply sums the unclaimed rewards over [`recentFeePeriods`](#recentfeeperiods) except those from the current period, because they cannot yet be claimed.
...
The message sender claims their fees in `xUSD`.
...
The message sender claims fees in `xUSD` for a specified address; the funds are remitted to that address, and not to the sender.
...
Allows the contract owner to escrow DOWS rewards for particular accounts. The rewards are escrowed for one year.

The DOWS is deposited into the [`RewardEscrow`](RewardEscrow.md) contract from the sender using the ERC20 transferFrom function. The tokens are then escrowed on behalf of the targeted account with [`RewardEscrow.appendVestingEntry`](RewardEscrow.md#appendvestingentry).
...
Allows the contract owner to set the [`Shadows` contract address](#shadows).
...
??? example "Details"
    ...
    * `_shadows` must not be the zero address.
...
This function merely emits an event and passes through to [`FeePoolState.appendAccountIssuanceRecord`](FeePoolState.md#appendAccountIssuanceRecord) and is itself only invoked by [`Shadows._appendAccountIssuanceRecord`](Shadows.md#_appendaccountissuancerecord).
...
Allows the [`Shadows._internalExchange`](Shadows.md#_internalexchange) function to record that a fee was paid whenever an exchange between Synth flavours occurs.

Adds the value in xUSD to the current fee period's pool of fees to be distributed.
...
Adds a quantity of DOWS to the current fee period's total of rewards to be distributed.
...
The account's collateralisation ratio must be less than the [issuance ratio](ShadowsState.md#issuanceratio), plus the [target threshold](#target_threshold), as specified by the [`feesClaimable`](#feesclaimable) function. The quantity of fees and rewards owed is computed by [`feesAvailable`](#feesavailable).
...
??? example "Details"
    ...
    * The user's [collateralisation ratio](Shadows.md#collateralisationratio) must be below the threshold, as per [`feesClaimable`](#feesclaimable).
    * The user must have a positive value of fees or rewards available to claim.

    **Emits**

    * [`FeesClaimed(claimingAddress, feesPaid, rewardsPaid)`](#feesclaimed) (`feesPaid` is denominated in xUSD, `rewardsPaid` in DOWS)
...
If $\Delta_i$ is the value of the $i^{th}$ entry in the [debt ledger](ShadowsState.md#debtledger) and $\omega$ is the provided debt ownership percentage, then the result of this function is:

$$
\omega \frac{\Delta_\text{exit}}{\Delta_\text{entry}}
$$

See [`Shadows._addToDebtRegister`](Shadows.md#_addToDebtRegister) for details of the debt ownership percentage adjustment.
...
Computes the fees (in xUSD) and rewards (in DOWS) owed at the end of a recent fee period given an entry index and the percentage of total system debt owned.
...
Pays a quantity of fees in xUSD to a claiming address.
...
??? example "Details"
    ...
    **Signature**
    ...
    `_payFees(address account, uint xUSDAmount) internal`
    ...
    * `account` can't be the Shadows contract.
...
??? example "Details"
    ...
    **Signature**
    ...
    `_payRewards(address account, uint dowsAmount) internal`
...
```python
remaining = xUSDAmount # The quantity to pay out.
...
??? example "Details"
    ...
    **Signature**
    ...
    `_recordFeePayment(uint xUSDAmount) internal returns (uint)`:

---

### `_recordRewardPayment`

Claims a quantity of DOWS rewards from the [recent fee periods](#recentfeeperiods). This is only called in `_claimFees`.

Its logic is identical to [`_recordFeePayment`](#_recordfeepayment), except that the relevant quantities are in `DOWS`, and are claimed from [`rewardsClaimed`](#feeperiod).

??? example "Details"

    **Signature**

    `_recordRewardPayment(uint dowsAmount) internal returns (uint)`:
...
Reverts the transaction if `msg.sender` is not the [`shadows`](#shadows) address.
...
**Signature:** `FeesClaimed(address account, uint xUSDAmount, uint dowsRewards)`
