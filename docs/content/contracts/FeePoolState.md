...
!!! caution "Caution: The Number of Stored Fee Periods"
    ...
    Note that this contract contains storage for [up to six fee periods](#fee_period_length), while the FeePool contract limits it to [only three](FeePool.md#fee_period_length). This is a consequence of the implementation of [SIP 4](https://sips.shadows.io/sips/sip-4), which reduced the fee window in the main [`FeePool`](FeePool.md) contract in order to encourage faster responses to alterations of system incentives. As part of this process, this storage contract was, of course, not upgraded.

    See also: [Design_Decisions.md](https://github.com/Shadowsio/shadows/blob/master/Design_Decisions.md#feepoolstate).

**Source:** [FeePoolState.sol](https://github.com/Shadowsio/shadows/blob/master/contracts/FeePoolState.sol)
...
| -------------- | ------ | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ |
| debtPercentage | `uint` | The percentage of the total system debt owned by the address associated with this entry at the time of issuance. These are [27-decimal fixed point numbers](SafeDecimalMath.md), closely related to the values in [`ShadowsState.debtLedger`](ShadowsState.md#debtledger). |
| debtEntryIndex | `uint` | The [debt ledger](ShadowsState.md#debtledger) index when this user issued or destroyed tokens. That is, the length of the ledger at the time of issuance.                                                                                                                    |

For more information on these fields and their meaning, see the main [`Shadows`](Shadows.md) contract functions [`_addToDebtRegister`](Shadows.md#_addtodebtregister) and [`_removeFromDebtRegister`](Shadows.md#_removefromdebtregister), along with the corresponding struct in [`ShadowsState`](ShadowsState.md#issuancedata).

!!! info "Relationship with `ShadowsState`"

    This is the same struct as [`ShadowsState.issuanceData`](ShadowsState.md#issuancedata), modulo naming, but in the case of ShadowsState, only one entry is kept, corresponding to only the most recent issuance event associated with an address.
...
Allows the [`Shadows`](Shadows.md#_appendaccountissuancerecord) contract, through [`FeePool.appendAccountIssuanceRecord`](FeePool.md#appendaccountissuancerecord), to record current fee period issuance information for a given account in the issuance ledger. This is used when synths are issued or burnt.
...
This function was used during the initial six week setup period to initialise the issuance ledger from the previous Shadows version.
