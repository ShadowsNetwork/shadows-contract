...
Defines the Shadows inflationary supply schedule, according to which the shadows inflationary supply is released.

Minting is performed in increments of a week whenever [`recordMintEvent`](#recordMintEvent) is called from [`Shadows.mint`](Shadows.md#mint). Minting can be called weekly after the time elapses for more than 7 days. These accrue so that no tokens are lost even if minting is not performed for several periods; the accrued total is minted at the next invocation. These computations are covered in more detail in the [`mintableSupply`](#mintablesupply) description.
...
After Week 235 the DOWS supply will grow at 2.5% APR based on the total Supply of DOWS each week. Compounded weekly the supply growth APY is 2.53% for the terminal supply growth.

**Source:** [SupplySchedule.sol](https://github.com/Shadowsio/shadows/blob/master/contracts/SupplySchedule.sol)
...
The address of the main [`ShadowsProxy`](Proxy.md) contract.
...
The timestamp at which the inflationary DOWS supply began to be minted.
...
A buffer added to the lastMintEvent to ensure that shadows rewards are issued after a feePeriod closes.
...
Used as the quantity of DOWS to reward the caller of [`Shadows.mint`](Shadows.md#mint), which incentivises users to continue minting the inflationary supply over time. Initialised to 200 DOWS.
...
Allows the owner to set the [`shadows`](#shadows) address.

??? example "Details"

    **Signature**

    `setShadows(IShadows _shadowsProxy) external`
...
This is called within [`Shadows.mint`](Shadows.md#mint) to declare that the outstanding inflationary supply of tokens has been minted before they are actually distributed.
...
Although this function has no check that any tokens actually are mintable when it is called, the [`Shadows`](Shadows.md) contract requires it to be the case, so double calls should not occur. Similarly, the function does not itself enforce that the actual token supply has been increased by [`Shadows`](Shadows.md) in a manner consistent with the defined schedule and must simply trust that this compact is observed.
...
Reverts the transaction if `msg.sender` is not the [`shadows`](#shadows) address. Shadows address is found by lookup to the proxy.target().
