...
module.exports = async function(deployer, network, accounts) {
	...
	// constructor(address _proxy, address _owner, Shadows _synthetix, address _feeAuthority, uint _transferFeeRate, uint _exchangeFeeRate)
	...
	// Shadows State
	// ----------------
	console.log('Deploying ShadowsState...');
	// constructor(address _owner, address _associatedContract)
	deployer.link(SafeDecimalMath, ShadowsState);
	const synthetixState = await deployer.deploy(ShadowsState, owner, ZERO_ADDRESS, {
		from: deployerAccount,
	});
	...
	// Shadows
	// ----------------
	console.log('Deploying ShadowsProxy...');
	// constructor(address _owner)
	const synthetixProxy = await Proxy.new(owner, { from: deployerAccount });

	console.log('Deploying ShadowsTokenState...');
	// constructor(address _owner, address _associatedContract)
	const synthetixTokenState = await TokenState.new(owner, deployerAccount, {
		from: deployerAccount,
	});

	console.log('Deploying Shadows...');
	// constructor(address _proxy, TokenState _tokenState, Shadows _synthetixState,
	...
	console.log('Deploying ShadowsEscrow...');
	await deployer.deploy(ShadowsEscrow, owner, synthetix.address, {
		from: deployerAccount,
	});
	...
	// Connect Escrow
	// ----------------------
	await synthetix.setEscrow(ShadowsEscrow.address, { from: owner });

	// ----------------------
	// Connect FeePool
	// ----------------------
	await feePool.setShadows(synthetix.address, { from: owner });
	...
	for (const currencyKey of currencyKeys) {
		...
		// ----------------------
		console.log(`Adding ${currencyKey} to Shadows contract...`);
	...
	const tableData = [
		...
		['Shadows State', synthetixState.address],
		['Shadows Token State', synthetixTokenState.address],
		['Shadows Proxy', synthetixProxy.address],
