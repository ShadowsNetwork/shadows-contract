...
contract('Shadows', async function(accounts) {
	...
	beforeEach(async function() {
		...
		synthetixState = await ShadowsState.at(await synthetix.synthetixState());
	...
	it('should allow the owner to set a TokenState contract', async function() {
		const transaction = await synthetix.setShadowsState(account1, { from: owner });
	...
	it('should error when relying on a stale exchange rate in effectiveValue()', async function() {
		...
		assert.bnEqual(await synthetix.effectiveValue(SNX, amountOfShadowss, sEUR), amountOfEur);
	...
	it('should not allow transfer of synthetix in escrow', async function() {
		...
		await synthetix.transfer(escrow.address, escrowedShadowss, { from: owner });
	...
	it("should allow me to burn all synths I've issued when there are other issuers", async function() {
		...
		await synthetix.transfer(account1, account1Shadowss, { from: owner }); // Issue the massive majority to account1
		await synthetix.transfer(account2, account2Shadowss, { from: owner }); // Issue a small amount to account2
	...
	it("should correctly calculate a user's maximum issuable synths without prior issuance", async function() {
		...
		await synthetix.transfer(account1, toUnit(issuedShadowss), { from: owner });
	...
	it('should not be possible to transfer locked synthetix', async function() {
		...
		await assert.revert(synthetix.transfer(account2, toUnit(issuedShadowss), { from: account1 }));
	});
	...
	it("should lock newly received synthetix if the user's collaterisation is too high", async function() {
		...
		const issuedShadowss = web3.utils.toBN('200000');
		await synthetix.transfer(account1, toUnit(issuedShadowss), { from: owner });
		await synthetix.transfer(account2, toUnit(issuedShadowss), { from: owner });
	...
	it("should include escrowed synthetix when calculating a user's collaterisation ratio", async function() {
		...
		await synthetix.transfer(account1, transferredShadowss, { from: owner });
		...
		const expectedCollaterisationRatio = divideDecimal(
			maxIssuable,
			multiplyDecimal(escrowedShadowss.add(transferredShadowss), snx2usdRate)
		);
	...
	// TODO: Are we testing too much Synth functionality here in Shadows
