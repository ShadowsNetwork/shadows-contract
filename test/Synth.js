...
contract('Synth', async function(accounts) {
	...
	beforeEach(async function() {
		...
		synthetix = await Shadows.deployed();
	...
	it('should set constructor params on deployment', async function() {
		// constructor(address _proxy, TokenState _tokenState, Shadows _synthetix, FeePool _feePool,
		...
		assert.equal(await synth.synthetix(), Shadows.address);
	...
	it('should allow the owner to set the Shadows contract', async function() {
		assert.notEqual(await XDRContract.synthetix(), account1);

		const transaction = await XDRContract.setShadows(account1, { from: owner });
		assert.eventEqual(transaction, 'ShadowsUpdated', { newShadows: account1 });

		assert.equal(await XDRContract.synthetix(), account1);
	});

	it('should disallow a non-owner from setting the Shadows contract', async function() {
		await assert.revert(XDRContract.setShadows(account1, { from: account1 }));
	});
	...
	it('should issue successfully when called by Shadows', async function() {
		// Set it to us so we can call it easily
		await XDRContract.setShadows(owner, { from: owner });
	...
	it('should revert when issue is called by non-Shadows address', async function() {
		await XDRContract.setShadows(synthetix.address, { from: owner });
		await assert.revert(XDRContract.issue(account1, toUnit('10000'), { from: owner }));
	});

	it('should burn successfully when called by Shadows', async function() {
	...
	it('should revert when burn is called by non-Shadows address', async function() {
		// Issue a bunch of synths so we can play with them.
		await synthetix.issueSynths(XDR, toUnit('10000'), { from: owner });

		// Set the synthetix reference to account1
		await XDRContract.setShadows(account1, { from: owner });
	...
	it('should triggerTokenFallback successfully when called by Shadows', async function() {
		// Set the synthetix reference to us so we can call it easily
		await XDRContract.setShadows(owner, { from: owner });
	...
	it('should revert on triggerTokenFallback when called by non-Shadows and non-FeePool address', async function() {
		await assert.revert(
