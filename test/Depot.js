...
contract('Depot', async function(accounts) {
	...
	it('should set synthetix when invoked by owner', async function() {
		const transaction = await depot.setShadows(address4, { from: owner });
		assert.eventEqual(transaction, 'ShadowsUpdated', { newShadowsContract: address4 });

		assert.equal(await depot.synthetix(), address4);
	});

	it('should not set synthetix when not invoked by owner', async function() {
		await assert.revert(depot.setShadows(owner, { from: deployerAccount }));
	});
	...
	describe('Ensure user can exchange ETH for Shadows', async function() {
		...
		it('ensure user get the correct amount of SNX after sending ETH', async function() {
			...
			// Purchaser sends ETH
			await depot.exchangeEtherForShadows({
				from: purchaser,
				value: ethToSend,
			});
			...
			const purchaseValueInShadows = divideDecimal(purchaseValueInSynthsAfterFees, snxUSD);
			...
			assert.bnEqual(purchaserSNXEndBalance, purchaseValueInShadows);
		});
	});
	...
	describe('Ensure user can exchange Synths for Shadows', async function() {
		...
		it('ensure user gets the correct amount of SNX after sending 10 sUSD', async function() {
			...
			const txn = await depot.exchangeSynthsForShadows(synthsToSend, {
				from: purchaser,
			});
