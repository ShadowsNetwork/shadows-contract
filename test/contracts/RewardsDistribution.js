...
contract('RewardsDistribution', async accounts => {
	...
	let rewardsDistribution, shadows, feePool, mockRewardsRecipient;
	...
	it('should set constructor params on deployment', async () => {
		...
		assert.equal(await instance.shadowsProxy(), account3);
	...
	describe('when the authority is distributing rewards', async () => {
		beforeEach(async () => {
			...
			// Set the DOWS Token Transfer Address
			await rewardsDistribution.setShadowsProxy(shadows.address, {
				from: owner,
			});
		...
		it('should send the correct amount of tokens to the listed addresses', async () => {
			...
			// Transfer DOWS to the RewardsDistribution contract address
			await shadows.methods['transfer(address,uint256)'](
			...
			const balanceOfRewardsContract = await shadows.balanceOf(rewardsDistribution.address);
			assert.bnEqual(balanceOfRewardsContract, totalToDistribute);

			// Distribute Rewards called from Shadows contract as the authority to distribute
			...
			const balanceOfAccount1 = await shadows.balanceOf(account1);
			assert.bnEqual(balanceOfAccount1, toUnit('5000'));

			// Check Account 2 balance
			const balanceOfAccount2 = await shadows.balanceOf(account2);
			assert.bnEqual(balanceOfAccount2, toUnit('10000'));

			// Check Reward Escrow balance
			const balanceOfRewardEscrow = await shadows.balanceOf(rewardEscrowAddress);
		...
		it('should call the notifyRewardAmount on mockRewardsRecipient', async () => {
			...
			// Transfer DOWS to the RewardsDistribution contract address
			await shadows.transfer(rewardsDistribution.address, totalToDistribute, { from: owner });
			...
			// Check Account 2 balance
			const balanceOfMockRewardsRecipient = await shadows.balanceOf(mockRewardsRecipient.address);
