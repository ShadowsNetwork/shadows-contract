...
contract('RewardEscrow', async accounts => {
	...
	const [DOWS] = ['DOWS'].map(toBytes32);

	const [, owner, feePoolAccount, account1, account2] = accounts;
	let rewardEscrow, shadows, exchangeRates, oracle;
	...
	describe('Constructor & Settings ', async () => {
		it('should set shadows on contructor', async () => {
			const shadowsAddress = await rewardEscrow.shadows();
			assert.equal(shadowsAddress, Shadows.address);
		});
		...
		it('should allow owner to set shadows', async () => {
			await rewardEscrow.setShadows(ZERO_ADDRESS, { from: owner });
			const shadowsAddress = await rewardEscrow.shadows();
			assert.equal(shadowsAddress, ZERO_ADDRESS);
		});
	...
	describe('Functions', async () => {
		...
		describe('Vesting Schedule Writes', async () => {
			it('should not create a vesting entry with a zero amount', async () => {
				// Transfer of DOWS to the escrow must occur before creating an entry
				await shadows.transfer(RewardEscrow.address, toUnit('1'), {
					from: owner,
				});
			...
			it('should not create a vesting entry if there is not enough DOWS in the contracts balance', async () => {
				// Transfer of DOWS to the escrow must occur before creating an entry
				await shadows.transfer(RewardEscrow.address, toUnit('1'), {
					from: owner,
				});
		...
		describe('Vesting Schedule Reads ', async () => {
			beforeEach(async () => {
				// Transfer of DOWS to the escrow must occur before creating a vestinng entry
				await shadows.transfer(RewardEscrow.address, toUnit('6000'), {
					from: owner,
				});
			...
			it('should append a vesting entry and increase the contracts balance', async () => {
				const balanceOfRewardEscrow = await shadows.balanceOf(RewardEscrow.address);
		...
		describe('Partial Vesting', async () => {
			beforeEach(async () => {
				...
				await exchangeRates.updateRates([DOWS], ['0.1'].map(toUnit), await currentTime(), {
					from: oracle,
				});
		...
		describe('Vesting', async () => {
			...
			it('should vest and transfer dows from contract to the user', async () => {
				await rewardEscrow.vest({ from: account1 });

				// Check user has all their vested DOWS
				assert.bnEqual(await shadows.balanceOf(account1), toUnit('6000'));

				// Check rewardEscrow does not have any DOWS
				assert.bnEqual(await shadows.balanceOf(RewardEscrow.address), toUnit('0'));
			});
		...
		describe('Stress Test', async () => {
			it('should not create more than MAX_VESTING_ENTRIES vesting entries', async () => {
				...
				// Transfer of DOWS to the escrow must occur before creating an entry
				await shadows.transfer(RewardEscrow.address, toUnit('260'), {
					from: owner,
				});
			...
			it('should be able to vest 52 week * 5 years vesting entries', async () => {
				...
				// Check user has all their vested DOWS
				assert.bnEqual(await shadows.balanceOf(account1), toUnit('260'));
			...
			it('should be able to read an accounts schedule of 5 vesting entries', async () => {
				// Transfer of DOWS to the escrow must occur before creating an entry
				await shadows.transfer(RewardEscrow.address, toUnit('5'), {
					from: owner,
				});
