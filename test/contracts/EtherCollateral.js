...
contract('EtherCollateral', async accounts => {
	...
	const issueSynthxUSD = async (issueAmount, receiver) => {
		// We need the owner to issue synths
		await shadows.issueSynths(issueAmount, { from: owner });
		// Set up the depositor with an amount of synths to deposit.
		await xUSDSynth.transfer(receiver, issueAmount, {
			from: owner,
		});
	};
	...
	const depositUSDInDepot = async (synthsToDeposit, depositor) => {
		...
		// Get xUSD from Owner
		await issueSynthxUSD(synthsToDeposit, depositor);

		// Approve Transaction
		await xUSDSynth.approve(depot.address, synthsToDeposit, { from: depositor });

		// Deposit xUSD in Depot
	...
	const calculateLoanFeexUSD = async feesInETH => {
		// Ask the Depot how many xUSD I will get for this ETH
		const expectedFeexUSD = await depot.synthsReceivedForEther(feesInETH);
		// console.log('expectedFeexUSD', expectedFeexUSD.toString());
		return expectedFeexUSD;
	};

	const updateRatesWithDefaults = async () => {
		const timestamp = await currentTime();

		// Depot requires DOWS and ETH rates
	...
	beforeEach(async () => {
		...
		xUSDSynth = await Synth.at(await shadows.synths(xUSD));
		xETHSynth = await MultiCollateralSynth.at(await shadows.synths(xETH));
	...
	describe('On deployment of Contract', async () => {
		...
		it('should access its dependencies via the address resolver', async () => {
			assert.equal(await addressResolver.getAddress(toBytes32('SynthxETH')), xETHSynth.address);
			assert.equal(await addressResolver.getAddress(toBytes32('SynthxUSD')), xUSDSynth.address);
	...
	describe('when accessing the external views then', async () => {
		it('collateralAmountForLoan should return 7500 ETH required to open 5000 xETH', async () => {
		...
		it('loanAmountFromCollateral should return 5000 xETH when opening a loan with 7500 ETH', async () => {
			const loanAmountFromCollateral = await etherCollateral.loanAmountFromCollateral(
				toUnit('7500')
			);
			// There is a rounding up. You get more xETH rather than less for your loan
	...
	describe('when opening a Loan', async () => {
		describe('then revert when ', async () => {
			...
			it('attempting to issue more near the supply cap', async () => {
				// reduce the supply cap to 100 xETH
		...
		describe('then create loan and', async () => {
			...
			const expectedxETHLoanAmount = calculateLoanAmount(tenETH);
			...
			it('increase the totalIssuedSynths', async () => {
				assert.bnEqual(await etherCollateral.totalIssuedSynths(), expectedxETHLoanAmount);
			});
			it('emit a LoanCreated event', async () => {
				assert.eventEqual(openLoanTransaction, 'LoanCreated', {
					account: address1,
					loanID: 1,
					amount: expectedxETHLoanAmount,
				});
			});
			...
			it('store the synthLoan.loanAmount', async () => {
				const synthLoan = await etherCollateral.getLoan(address1, loanID);
				assert.bnEqual(synthLoan.loanAmount, expectedxETHLoanAmount);
			});
			...
			it('add the loan issue amount to creators balance', async () => {
				const xETHBalance = await xETHSynth.balanceOf(address1);
				assert.bnEqual(xETHBalance, expectedxETHLoanAmount);
			});
			...
			describe('when opening a second loan against address1', async () => {
				...
				const expectedxETHLoanAmount = calculateLoanAmount(fiveThousandETH);
				...
				it('then increase the totalIssuedSynths', async () => {
					assert.bnEqual(
						...
						totalIssuedSynthsBefore.add(expectedxETHLoanAmount)
					);
				});
				...
				describe('when opening a third loan against address2', async () => {
					...
					const expectedxETHLoanAmount = calculateLoanAmount(threeNintyETH);
					...
					it('then increase the totalIssuedSynths', async () => {
						assert.bnEqual(
							await etherCollateral.totalIssuedSynths(),
							totalSupplyBefore.add(expectedxETHLoanAmount)
						);
					});
					...
					describe('when closing the first loan of address1', async () => {
						...
						beforeEach(async () => {
							// Deposit xUSD in Depot to allow fees to be bought with ETH
							...
							// expectedFeexUSD = await calculateLoanFeexUSD(expectedFeeETH);
							interestRatePerSec = await etherCollateral.interestPerSecond();
							// Get the total xETH Issued
							totalIssuedSynthsBefore = await etherCollateral.totalIssuedSynths();
							// Close loan
		...
		describe('when closing a Loan', async () => {
			describe('then it reverts when', async () => {
				...
				it('xETH balance is less than loanAmount', async () => {
					// "Burn" some of accounts xETH by sending to the owner
					await xETHSynth.transfer(owner, toUnit('4'), { from: address1 });
					await assert.revert(etherCollateral.closeLoan(loanID, { from: address1 }));
				});

				it('Depot has no xUSD to buy for Fees', async () => {
					// Dont put any xUSD into the Depot and close the loan
					await assert.revert(etherCollateral.closeLoan(loanID, { from: address1 }));
				});
			});
			...
			describe('then it closes the loan and', async () => {
				...
				const oneThousandxUSD = toUnit('1000');
				...
				beforeEach(async () => {
					...
					// Deposit xUSD in Depot to allow fees to be bought with ETH
					await depositUSDInDepot(oneThousandxUSD, depotDepositor);
				...
				it('reduce xETH totalSupply', async () => {
					assert.bnEqual(await etherCollateral.totalIssuedSynths(), ZERO_BN);
				});

				it('increase the FeePool xUSD balance', async () => {
					assert.bnEqual(await xUSDSynth.balanceOf(FEE_ADDRESS), expectedFeexUSD);
				});

				it('record the fees in the FeePool.feesToDistribute', async () => {
					const currentFeePeriod = await feePoolProxy.recentFeePeriods(0);
					assert.bnEqual(currentFeePeriod.feesToDistribute, expectedFeexUSD);
				});
				...
				it('decrease the xUSD balance in Depot', async () => {
					const expectedBalance = oneThousandxUSD.sub(expectedFeexUSD);
					assert.bnEqual(await xUSDSynth.balanceOf(depot.address), expectedBalance);
				});
