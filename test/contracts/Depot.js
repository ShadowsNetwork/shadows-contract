...
contract('Depot', async accounts => {
	let shadows, synth, depot, addressResolver, exchangeRates;
	...
	const [xUSD, xAUD, xEUR, xBTC, DOWS, iBTC, xETH, ETH] = [
		'xUSD',
		'xAUD',
		'xEUR',
		'xBTC',
	...
	const updateRatesWithDefaults = async () => {
		...
		await exchangeRates.updateRates(
			[xAUD, xEUR, DOWS, xBTC, iBTC, xETH, ETH],
	...
	const approveAndDepositSynths = async (synthsToDeposit, depositor) => {
		// Approve Transaction
		// console.log('Approve Transaction on xUSD');
		await synth.approve(depot.address, synthsToDeposit, { from: depositor });

		// Deposit xUSD in Depot
		// console.log('Deposit xUSD in Depot amount', synthsToDeposit, depositor);
	...
	beforeEach(async () => {
		...
		synth = await Synth.at(await shadows.synths(xUSD));
	...
	describe('Restricted methods', () => {
		it('ensure only known functions are mutative', () => {
			ensureOnlyExpectedMutativeFunctions({
				...
				expected: [
					'depositSynths',
					'exchangeEtherForDOWS',
					'exchangeEtherForDOWSAtRate',
					'exchangeEtherForSynths',
					'exchangeEtherForSynthsAtRate',
					'exchangeSynthsForDOWS',
					'exchangeSynthsForDOWSAtRate',
					'setFundsWallet',
					'setMaxEthPurchase',
	...
	describe('Ensure user can exchange ETH for Synths where the amount', async () => {
		...
		it('exactly matches one deposit (and that the queue is correctly updated)', async () => {
			...
			// Exchange("ETH", msg.value, "xUSD", fulfilled);
	...
	describe('Ensure user can exchange ETH for Shadows', async function() {
		const purchaser = address1;
		let depot;
		let shadows;
		const ethUSD = web3.utils.toWei('500');
		const dowxUSD = web3.utils.toWei('.10');
		...
		this.beforeEach(async () => {
			...
			await exchangeRates.updateRates([DOWS, ETH], [dowxUSD, ethUSD], timestamp, {
				from: oracle,
			});

			// We need to send some DOWS to the Token Depot contract
			await shadows.transfer(depot.address, web3.utils.toWei('1000000'), {
				from: owner,
			});
		});

		it('ensure user get the correct amount of DOWS after sending ETH', async () => {
			const ethToSend = toUnit('10');

			const purchaserDOWSStartBalance = await shadows.balanceOf(purchaser);
			// Purchaser should not have DOWS yet
			assert.equal(purchaserDOWSStartBalance, 0);

			// Purchaser sends ETH
			await depot.exchangeEtherForDOWS({
				from: purchaser,
				value: ethToSend,
			});

			const purchaseValueInSynths = multiplyDecimal(ethToSend, ethUSD);
			const purchaseValueInShadows = divideDecimal(purchaseValueInSynths, dowxUSD);

			const purchaserDOWSEndBalance = await shadows.balanceOf(purchaser);

			// Purchaser DOWS balance should be equal to the purchase value we calculated above
			assert.bnEqual(purchaserDOWSEndBalance, purchaseValueInShadows);
		});
	});

	describe('Ensure user can exchange Synths for Shadows', async function() {
		const purchaser = address1;
		const purchaserSynthAmount = toUnit('2000');
		const depotDOWSAmount = toUnit('1000000');
		let depot;
		let shadows;
		let synth;
		const dowxUSD = toUnit('.10');
		...
		this.beforeEach(async () => {
			...
			await exchangeRates.updateRates([DOWS], [dowxUSD], timestamp, {
				from: oracle,
			});

			// We need the owner to issue synths
			await shadows.issueSynths(toUnit('50000'), { from: owner });
			// Send the purchaser some synths
			await synth.transfer(purchaser, purchaserSynthAmount, {
				from: owner,
			});
			// We need to send some DOWS to the Token Depot contract
			await shadows.transfer(depot.address, depotDOWSAmount, {
				from: owner,
			});

			await synth.approve(depot.address, synthsToSend, { from: purchaser });

			const depotDOWSBalance = await shadows.balanceOf(depot.address);
			const purchaserSynthBalance = await synth.balanceOf(purchaser);
			assert.bnEqual(depotDOWSBalance, depotDOWSAmount);
			assert.bnEqual(purchaserSynthBalance, purchaserSynthAmount);
		});

		it('ensure user gets the correct amount of DOWS after sending 10 xUSD', async () => {
			const purchaserDOWSStartBalance = await shadows.balanceOf(purchaser);
			// Purchaser should not have DOWS yet
			assert.equal(purchaserDOWSStartBalance, 0);

			// Purchaser sends xUSD
			const txn = await depot.exchangeSynthsForDOWS(synthsToSend, {
				from: purchaser,
			});

			const purchaseValueInShadows = divideDecimal(synthsToSend, dowxUSD);
			...
			assert.eventEqual(exchangeEvent, 'Exchange', {
				...
				toCurrency: 'DOWS',
				toAmount: purchaseValueInShadows,
			});
		});
	});
});
