...
contract('Exchange Rates', async accounts => {
	...
	const [DOWS, xJPY, xXTZ, xBNB, xUSD, xEUR, xAUD] = [
		'DOWS',
		'xJPY',
		'xXTZ',
		'xBNB',
	...
	describe('constructor', () => {
		it('should set constructor params on deployment', async () => {
			...
			assert.etherEqual(await instance.rateForCurrency(xUSD), '1');
			assert.etherEqual(await instance.rateForCurrency(DOWS), '0.2');
			...
			const lastUpdatedTimeXUSD = await instance.lastRateUpdateTimes.call(xUSD);
			assert.isAtLeast(lastUpdatedTimeXUSD.toNumber(), creationTime);
			...
			const lastUpdatedTimeDOWS = await instance.lastRateUpdateTimes.call(DOWS);
			assert.isAtLeast(lastUpdatedTimeDOWS.toNumber(), creationTime);

			const xUSDRate = await instance.rateForCurrency(xUSD);
			assert.bnEqual(xUSDRate, toUnit('1'));
		});
		...
		it("shouldn't be able to set exchange rate to 0 on create", async () => {
			await assert.revert(
				ExchangeRates.new(owner, oracle, [DOWS], [web3.utils.toWei('0', 'ether')], {
					from: deployerAccount,
				})
			);
		});
	...
	describe('updateRates()', () => {
		...
		it('should revert when trying to set xUSD price', async () => {
			await fastForward(1);

			await assert.revert(
				instance.updateRates([xUSD], [web3.utils.toWei('1.0', 'ether')], timeSent, {
					from: oracle,
				})
			);
		});
	...
	describe('rateIsStale()', () => {
		it('should never allow xUSD to go stale via rateIsStale', async () => {
			await fastForward(await instance.rateStalePeriod());
			const rateIsStale = await instance.rateIsStale(xUSD);
	...
	describe('anyRateIsStale()', () => {
		it('should never allow xUSD to go stale via anyRateIsStale', async () => {
			const keysArray = [DOWS, toBytes32('GOLD')];
			...
			// Even though xUSD hasn't been updated since the stale rate period has expired,
			// we expect that xUSD remains "not stale"
			assert.equal(await instance.anyRateIsStale(keysArray), false);
		});
	...
	describe('effectiveValue()', () => {
		...
		describe('when a price is sent to the oracle', () => {
			...
			it('should correctly calculate an exchange rate in effectiveValue()', async () => {
				// 1 xUSD should be worth 2 xAUD.
				assert.bnEqual(await instance.effectiveValue(xUSD, toUnit('1'), xAUD), toUnit('2'));

				// 10 DOWS should be worth 1 xUSD.
				assert.bnEqual(await instance.effectiveValue(DOWS, toUnit('10'), xUSD), toUnit('1'));

				// 2 xEUR should be worth 2.50 xUSD
				assert.bnEqual(await instance.effectiveValue(xEUR, toUnit('2'), xUSD), toUnit('2.5'));
			});
			...
			it('should error when relying on a stale exchange rate in effectiveValue()', async () => {
				...
				// Update all rates except xUSD.
				await instance.updateRates([xEUR, DOWS], ['1.25', '0.1'].map(toUnit), timestamp, {
					from: oracle,
				});

				const amountOfShadowss = toUnit('10');
				const amountOfEur = toUnit('0.8');

				// Should now be able to convert from DOWS to xEUR since they are both not stale.
				assert.bnEqual(await instance.effectiveValue(DOWS, amountOfShadowss, xEUR), amountOfEur);

				// But trying to convert from DOWS to xAUD should fail as xAUD should be stale.
				await assert.revert(instance.effectiveValue(DOWS, toUnit('10'), xAUD));
				await assert.revert(instance.effectiveValue(xAUD, toUnit('10'), DOWS));
			});
			...
			it('should revert when relying on a non-existant exchange rate in effectiveValue()', async () => {
				...
				await assert.revert(instance.effectiveValue(DOWS, toUnit('10'), toBytes32('XYZ')));
			});
		});
	});

	describe('inverted prices', () => {
		const inverseRates = ['iBTC', 'iETH', 'xEUR', 'xBNB'];
		const [iBTC, iETH, xEUR, xBNB] = inverseRates.map(toBytes32);
		it('rateIsFrozen for a regular synth returns false', async () => {
			assert.equal(false, await instance.rateIsFrozen(xEUR));
		});
		...
		describe('when two inverted synths are added', () => {
			...
			describe('when updateRates is called with an in-bounds update', () => {
				...
				beforeEach(async () => {
					...
					const timeSent = await currentTime();
					txn = await instance.updateRates([iBTC, iETH, xEUR, xBNB], rates, timeSent, {
						from: oracle,
					});
				});
				it('regular and inverted rates should be updated correctly', async () => {
					await assertRatesAreCorrect({
						txn,
						currencyKeys: [iBTC, iETH, xEUR, xBNB],
			...
			describe('when updateRates is called with an upper out-of-bounds update', () => {
				...
				describe('when a regular (non-inverse) synth is removed by the owner', () => {
					it('then it reverts', async () => {
						await assert.revert(
							instance.removeInversePricing(xEUR, {
								from: owner,
							})
						);
						await assert.revert(
							instance.removeInversePricing(xBNB, {
								from: owner,
							})
						);
					});
	...
	describe('pricing aggregators', () => {
		it('only an owner can add an aggregator', async () => {
			await onlyGivenAddressCanInvoke({
				...
				args: [xJPY, aggregatorJPY.address],
				accounts,
				address: owner,
			});
		});
		...
		describe('when the owner attempts to add an invalid address for xJPY ', () => {
			it('then zero address is invalid', async () => {
				await assert.revert(
					instance.addAggregator(xJPY, ZERO_ADDRESS, {
						from: owner,
					})
				);
			});
			it('and a non-aggregator address is invalid', async () => {
				await assert.revert(
					instance.addAggregator(xJPY, instance.address, {
						from: owner,
					})
				);
			});
		...
		describe('when the owner adds xJPY added as an aggregator', () => {
			let txn;
			beforeEach(async () => {
				txn = await instance.addAggregator(xJPY, aggregatorJPY.address, {
					from: owner,
				});
			});

			it('then the list of aggregatorKeys lists it', async () => {
				assert.equal('xJPY', bytesToString(await instance.aggregatorKeys(0)));
			...
			describe('when the owner tries to remove an invalid aggregator', () => {
				it('then it reverts', async () => {
					await assert.revert(
						instance.removeAggregator(xXTZ, { from: owner }),
						'No aggregator exists for key'
					);
				});
			});

			describe('when the owner adds xXTZ as an aggregator', () => {
				beforeEach(async () => {
					txn = await instance.addAggregator(xXTZ, aggregatorXTZ.address, {
						from: owner,
					});
				});
				...
				it('then the list of aggregatorKeys lists it also', async () => {
					assert.equal('xJPY', bytesToString(await instance.aggregatorKeys(0)));
					assert.equal('xXTZ', bytesToString(await instance.aggregatorKeys(1)));
				...
				describe('when the ratesAndStaleForCurrencies is queried', () => {
					let response;
					beforeEach(async () => {
						response = await instance.ratesAndStaleForCurrencies([xJPY, xXTZ]);
					});
				...
				describe('when the aggregator price is set for xJPY', () => {
					const newRate = 111;
					let timestamp;
					beforeEach(async () => {
						timestamp = await currentTime();
					...
					describe('when the aggregator price is set for xXTZ', () => {
						...
						describe('when the aggregator is removed for xJPY', () => {
							beforeEach(async () => {
								txn = await instance.removeAggregator(xJPY, {
									from: owner,
								});
							});
							...
							describe('when a user queries the aggregatorKeys', () => {
								it('then only xXTZ is left', async () => {
									assert.equal('xXTZ', bytesToString(await instance.aggregatorKeys(0)));
									await assert.invalidOpcode(instance.aggregatorKeys(1));
								});
							});
							describe('when the ratesAndStaleForCurrencies is queried', () => {
			...
			describe('when the aggregator price is set to set a specific number (with support for 8 decimals)', () => {
				...
				describe('when the price is fetched for xJPY', () => {
					it('the specific number is returned with 18 decimals', async () => {
						const result = await instance.rateForCurrency(xJPY, {
							from: accountOne,
						});
						assert.bnEqual(result, toUnit(newRate.toString()));
					});
					it('and the timestamp is the latest', async () => {
						const result = await instance.lastRateUpdateTimes(xJPY, {
							from: accountOne,
						});
		...
		describe('when a price already exists for xJPY', () => {
			const oldPrice = 100;
			let timeOldSent;
			beforeEach(async () => {
				timeOldSent = await currentTime();

				await instance.updateRates([xJPY], [web3.utils.toWei(oldPrice.toString())], timeOldSent, {
					from: oracle,
				});
			});
			describe('when the ratesAndStaleForCurrencies is queried with xJPY', () => {
				let response;
				beforeEach(async () => {
					response = await instance.ratesAndStaleForCurrencies([xJPY]);
				});
			...
			describe('when the price is inspected for xJPY', () => {
			...
			describe('when xJPY added as an aggregator (replacing existing)', () => {
				beforeEach(async () => {
					await instance.addAggregator(xJPY, aggregatorJPY.address, {
						from: owner,
					});
				});
				describe('when the price is fetched for xJPY', () => {
				...
				describe('when the timestamp is fetched for xJPY', () => {
					it('0 is returned', async () => {
						const result = await instance.lastRateUpdateTimes(xJPY, {
							from: accountOne,
						});
				...
				describe('when the aggregator price is set to set a specific number (with support for 8 decimals)', () => {
					...
					describe('when the aggregator is removed for xJPY', () => {
						beforeEach(async () => {
							await instance.removeAggregator(xJPY, {
								from: owner,
							});
						});
						describe('when a user queries the first entry in aggregatorKeys', () => {
			...
			describe('when xXTZ added as an aggregator', () => {
				beforeEach(async () => {
					await instance.addAggregator(xXTZ, aggregatorXTZ.address, {
						from: owner,
					});
				});
				describe('when the ratesAndStaleForCurrencies is queried with xJPY and xXTZ', () => {
					let response;
					beforeEach(async () => {
						response = await instance.ratesAndStaleForCurrencies([xJPY, xXTZ]);
					});
					...
					it('with xXTZ having no value', () => {
						assert.bnEqual(response[0][0], web3.utils.toWei(oldPrice.toString()));
						assert.bnEqual(response[0][1], '0');
					});
				});

				describe('when the aggregator price is set to set for xXTZ', () => {
					const newRate = 99;
					let timestamp;
					beforeEach(async () => {
						await fastForward(50);
	...
	describe('roundIds for historical rates', () => {
		it('getCurrentRoundId() by default is 0 for all synths except xUSD which is 1', async () => {
			// Note: rates that were set in the truffle migration will be at 1, so we need to check
			// other synths
			assert.equal(await instance.getCurrentRoundId(xJPY), '0');
			assert.equal(await instance.getCurrentRoundId(xBNB), '0');
			assert.equal(await instance.getCurrentRoundId(xUSD), '1');
		});
		describe('given an aggregator exists for xJPY', () => {
			beforeEach(async () => {
				await instance.addAggregator(xJPY, aggregatorJPY.address, {
					from: owner,
				});
			...
			describe('and it has been given three successive rates a second apart', () => {
				...
				describe('and the xBNB rate (non-aggregator) has been set three times directly also', () => {
					let timestamp;

					beforeEach(async () => {
						for (let i = 0; i < 3; i++) {
							timestamp = 10000;
							await instance.updateRates([xBNB], [toUnit((1000 + i).toString())], timestamp + i, {
								from: oracle,
							});
						}
					});
					describe('getCurrentRoundId())', () => {
						describe('when invoked for an aggregator', () => {
							it('getCurrentRound() returns the last entry', async () => {
								await assert.equal((await instance.getCurrentRoundId(xJPY)).toString(), '3');
							});
						});
						describe('when invoked for a regular price', () => {
							it('getCurrentRound() returns the last entry', async () => {
								await assert.equal((await instance.getCurrentRoundId(xBNB)).toString(), '3');
							});
						});
					});
			...
			describe('and both the aggregator and regualr prices have been given three rates, 30seconds apart', () => {
				beforeEach(async () => {
					await aggregatorJPY.setLatestAnswer(convertToAggregatorPrice(100), 30); // round 1 for xJPY
					await aggregatorJPY.setLatestAnswer(convertToAggregatorPrice(200), 60); // round 2 for xJPY
					await aggregatorJPY.setLatestAnswer(convertToAggregatorPrice(300), 90); // round 3 for xJPY

					await instance.updateRates([xBNB], [toUnit('1000')], '30', { from: oracle }); // round 1 for xBNB
					await instance.updateRates([xBNB], [toUnit('2000')], '60', { from: oracle }); // round 2 for xBNB
					await instance.updateRates([xBNB], [toUnit('3000')], '90', { from: oracle }); // round 3 for xBNB
				});
				...
				describe('getLastRoundIdBeforeElapsedSecs()', () => {
					describe('when getLastRoundIdBeforeElapsedSecs() is invoked with the first round and a waiting time of less than 30s', () => {
						it('then it receives round 1 - no change ', async () => {
							...
							assert.equal(
								(await instance.getLastRoundIdBeforeElapsedSecs(xJPY, '1', 40, 10)).toString(),
								'1'
							);
							assert.equal(
								(await instance.getLastRoundIdBeforeElapsedSecs(xBNB, '1', 40, 10)).toString(),
								'1'
							);
						});
					});
					...
					describe('when getLastRoundIdBeforeElapsedSecs() is invoked with the first round and a waiting time of 30s exactly', () => {
						it('then it receives round 2 ', async () => {
							assert.equal(
								(await instance.getLastRoundIdBeforeElapsedSecs(xJPY, '1', 40, 20)).toString(),
								'2'
							);
							assert.equal(
								(await instance.getLastRoundIdBeforeElapsedSecs(xBNB, '1', 40, 20)).toString(),
								'2'
							);
						});
					});
					...
					describe('when getLastRoundIdBeforeElapsedSecs() is invoked with the second round and a waiting time of 30s exactly', () => {
						it('then it receives round 3', async () => {
							assert.equal(
								(await instance.getLastRoundIdBeforeElapsedSecs(xJPY, '2', 65, 25)).toString(),
								'3'
							);
							assert.equal(
								(await instance.getLastRoundIdBeforeElapsedSecs(xBNB, '2', 65, 25)).toString(),
								'3'
							);
						});
					});
					...
					describe('when getLastRoundIdBeforeElapsedSecs() is invoked with the first round and a waiting time between 30s to 60s', () => {
						it('then it receives round 2 ', async () => {
							assert.equal(
								(await instance.getLastRoundIdBeforeElapsedSecs(xJPY, '1', 40, 40)).toString(),
								'2'
							);
							assert.equal(
								(await instance.getLastRoundIdBeforeElapsedSecs(xBNB, '1', 40, 40)).toString(),
								'2'
							);
						});
					});
					describe('when getLastRoundIdBeforeElapsedSecs() is invoked with the first round and a waiting time of 60s exactly', () => {
						it('then it receives round 3 ', async () => {
							assert.equal(
								(await instance.getLastRoundIdBeforeElapsedSecs(xJPY, '1', 50, 40)).toString(),
								'3'
							);
							assert.equal(
								(await instance.getLastRoundIdBeforeElapsedSecs(xBNB, '1', 50, 40)).toString(),
								'3'
							);
						});
					});
					describe('when getLastRoundIdBeforeElapsedSecs() is invoked with the first round and a waiting time beyond 60s', () => {
						it('then it receives round 3 as well ', async () => {
							assert.equal(
								(await instance.getLastRoundIdBeforeElapsedSecs(xJPY, '1', 55, 6000)).toString(),
								'3'
							);
							assert.equal(
								(await instance.getLastRoundIdBeforeElapsedSecs(xBNB, '1', 50, 40)).toString(),
					...
					describe('when getLastRoundIdBeforeElapsedSecs() is invoked with the third round and a waiting time beyond 60s', () => {
						it('then it still receives round 3', async () => {
							assert.equal(
								(await instance.getLastRoundIdBeforeElapsedSecs(xJPY, '3', 180, 9000)).toString(),
								'3'
							);
							assert.equal(
								(await instance.getLastRoundIdBeforeElapsedSecs(xBNB, '1', 50, 40)).toString(),
			...
			describe('effectiveValueAtRound()', () => {
				describe('when both the aggregator and regular prices have been give three rates with current timestamps', () => {
					beforeEach(async () => {
						...
						await aggregatorJPY.setLatestAnswer(convertToAggregatorPrice(100), timestamp); // round 1 for xJPY
						await instance.updateRates([xBNB], [toUnit('1000')], timestamp, { from: oracle }); // round 1 for xBNB

						await fastForward(120);
						timestamp = await currentTime();
						await aggregatorJPY.setLatestAnswer(convertToAggregatorPrice(200), timestamp); // round 2 for xJPY
						await instance.updateRates([xBNB], [toUnit('2000')], timestamp, { from: oracle }); // round 2 for xBNB

						await fastForward(120);
						timestamp = await currentTime();
						await aggregatorJPY.setLatestAnswer(convertToAggregatorPrice(300), timestamp); // round 3 for xJPY
						await instance.updateRates([xBNB], [toUnit('4000')], timestamp, { from: oracle }); // round 3 for xBNB
					});
					it('accepts various changes to src roundId', async () => {
						assert.bnEqual(
							await instance.effectiveValueAtRound(xJPY, toUnit('1'), xBNB, '1', '1'),
							toUnit('0.1')
						);
						assert.bnEqual(
							await instance.effectiveValueAtRound(xJPY, toUnit('1'), xBNB, '2', '1'),
							toUnit('0.2')
						);
						assert.bnEqual(
							await instance.effectiveValueAtRound(xJPY, toUnit('1'), xBNB, '3', '1'),
							toUnit('0.3')
						);
					});
					it('accepts various changes to dest roundId', async () => {
						...
						assert.bnEqual(
							await instance.effectiveValueAtRound(xJPY, toUnit('1'), xBNB, '1', '2'),
							toUnit('0.05')
						);
						assert.bnEqual(
							await instance.effectiveValueAtRound(xJPY, toUnit('1'), xBNB, '1', '3'),
							toUnit('0.025')
						);
					});
					it('and combinations therein', async () => {
						assert.bnEqual(
							await instance.effectiveValueAtRound(xJPY, toUnit('1'), xBNB, '2', '2'),
							toUnit('0.1')
						);
						assert.bnEqual(
							await instance.effectiveValueAtRound(xJPY, toUnit('1'), xBNB, '3', '3'),
							toUnit('0.075')
						);
						assert.bnEqual(
							await instance.effectiveValueAtRound(xJPY, toUnit('1'), xBNB, '3', '2'),
							toUnit('0.15')
						);
					});
				});
