...
contract('SupplySchedule', async accounts => {
	...
	const [deployerAccount, owner, shadows, account1, account2] = accounts;

	let supplySchedule, shadowsProxy, decayRate;
	...
	beforeEach(async () => {
		...
		await shadowsProxy.setTarget(shadows, { from: owner });
	...
	describe('linking shadows', async () => {
		it('should have set shadows proxy', async () => {
			const shadowsProxy = await supplySchedule.shadowsProxy();
			assert.equal(shadowsProxy, shadowsProxy);
		});
		it('should revert when setting shadows proxy to ZERO_ADDRESS', async () => {
			await assert.revert(supplySchedule.setShadowsProxy(ZERO_ADDRESS, { from: owner }));
		});

		it('should emit an event when setting shadows proxy', async () => {
			const txn = await supplySchedule.setShadowsProxy(account2, { from: owner });

			assert.eventEqual(txn, 'ShadowsProxyUpdated', {
				newAddress: account2,
			});
		});
	});
	...
	describe('functions and modifiers', async () => {
		...
		describe('mintable supply', async () => {
			...
			async function checkMintedValues(
				...
				// call updateMintValues to mimic shadows issuing tokens
			...
			describe('setting weekCounter and lastMintEvent on supplySchedule to week 39', async () => {
				...
				beforeEach(async () => {
					...
					// setup new instance
					await instance.setShadowsProxy(shadowsProxy.address, { from: owner });
					await shadowsProxy.setTarget(shadows, { from: owner });
				});
