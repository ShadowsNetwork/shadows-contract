...
contract('MultiCollateralSynth', accounts => {
	...
	describe('when a MultiCollateral synth is added and connected to Shadows', () => {
		...
		describe('when non-multiCollateral tries to issue', () => {
			it('then it fails', async () => {
				await onlyGivenAddressCanInvoke({
					...
					accounts,
					reason: 'Only Shadows, FeePool, Exchanger, Issuer or MultiCollateral contracts allowed',
				});
			});
		});
		...
		describe('when multiCollateral is set to the owner', () => {
			...
			describe('when shadows set to account1', () => {
				beforeEach(async () => {
					// have account1 simulate being Shadows so we can invoke issue and burn
					await resolver.importAddresses([toBytes32('Shadows')], [account1], { from: owner });
				});
