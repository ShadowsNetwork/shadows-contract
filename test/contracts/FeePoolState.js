...
contract('FeePoolState', async accounts => {
	...
	const [xEUR, xAUD, xBTC, DOWS] = ['xEUR', 'xAUD', 'xBTC', 'DOWS'].map(toBytes32);
	...
	// TODO checks ShadowsState debt entry is same as stored FeePoolState Entry
	...
	// 	const maxIssuable = await shadows.maxIssuableSynths(account1, xUSD);
	...
	// 	const issuanceDataFromState = await shadowsState.issuanceData(account1);
	...
	// 	// Give some DOWS to account1
	// 	await shadows.transfer(account1, toUnit('20000'), { from: owner });

	// 	// Issue
	// 	await shadows.issueSynths(toUnit('200'), { from: account1 });
	...
	// 	issuanceDataFromState = await shadowsState.issuanceData(account1);
	...
	// 	// And account1 should own all of it.
	// 	assert.bnEqual(await shadows.debtBalanceOf(account1, xUSD), toUnit('400'));
