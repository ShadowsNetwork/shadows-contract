...
contract('Rewards Integration Tests', async accounts => {
	...
	beforeEach(async () => {
		...
		MINTER_DOWS_REWARD = await supplySchedule.minterReward();
		...
		// Assign 1/3 of total DOWS to 3 accounts
		const dowsTotalSupply = await shadows.totalSupply();
		const thirdOfDOWS = third(dowsTotalSupply);

		await shadows.transfer(account1, thirdOfDOWS, { from: owner });
		await shadows.transfer(account2, thirdOfDOWS, { from: owner });
		await shadows.transfer(account3, thirdOfDOWS, { from: owner });

		// Get the DOWS mintableSupply
		periodOneMintableSupplyMinusMinterReward = (await supplySchedule.mintableSupply()).sub(
			MINTER_DOWS_REWARD
		);

		// Mint the staking rewards
		await shadows.mint({ from: deployerAccount });
	});

	describe('3 accounts with 33.33% DOWS all issue MAX and claim rewards', async () => {
		...
		beforeEach(async () => {
			...
			await shadows.issueMaxSynths({ from: account2 });
			await shadows.issueMaxSynths({ from: account3 });
		});
		...
		it('should show the totalRewardsAvailable in the claimable periods 1 & 2', async () => {
			...
			for (let i = 0; i <= CLAIMABLE_PERIODS - 1; i++) {
				...
				// Get the DOWS mintableSupply - the minter reward of 200 DOWS
				mintedRewardsSupply = (await supplySchedule.mintableSupply()).sub(MINTER_DOWS_REWARD);
				// console.log('mintedRewardsSupply', mintedRewardsSupply.toString());
				// Mint the staking rewards
				await shadows.mint({ from: owner });
		...
		it('should mint DOWS for the all claimable fee periods then all 3 accounts claim at the end of the claimable period', async () => {
			let mintedRewardsSupply;
			// We are currently in the 2nd week, close it and the next
			for (let i = 0; i <= CLAIMABLE_PERIODS - 1; i++) {
				// console.log('Close Fee Period', i);
		...
		it('should rollover the unclaimed DOWS rewards', async () => {
		...
		it('should rollover the unclaimed DOWS rewards on week over 2 terms (FEE_PERIOD_LENGTH * 2)', async () => {
		...
		it('should rollover the partial unclaimed DOWS rewards', async () => {
		...
		it('should allow a user to leave the system and return and still claim rewards', async () => {
			...
			const burnableTotal = await shadows.debtBalanceOf(account1, xUSD);
			await shadows.burnSynths(burnableTotal, { from: account1 });
		...
		it('should allocate correct DOWS rewards as others leave the system', async () => {
			// Close Fee Period
			...
			// Get the DOWS mintableSupply for period 2
	...
	describe('Exchange Rate Shift tests', async () => {
		it('should assign accounts (1,2,3) to have (40%,40%,20%) of the debt/rewards', async () => {
			// Account 1&2 issue 10K USD and exchange in xBTC each, holding 50% of the total debt.
			await shadows.issueSynths(tenK, { from: account1 });
			await shadows.issueSynths(tenK, { from: account2 });

			await shadows.exchange(xUSD, tenK, xBTC, { from: account1 });
			await shadows.exchange(xUSD, tenK, xBTC, { from: account2 });
			...
			// Increase xBTC price by 100%
			const timestamp = await currentTime();
			await exchangeRates.updateRates([xBTC], ['10000'].map(toUnit), timestamp, {
				from: oracle,
			});

			// Account 3 (enters the system and) mints 10K xUSD (minus half of an exchange fee - to balance the fact
			// that the other two holders have doubled their xBTC holdings) and should have 20% of the debt not 33.33%
			const potentialFee = await feePool.exchangeFeeIncurred(toUnit('10000'));
			await shadows.issueSynths(tenK.sub(half(potentialFee)), { from: account3 });

			// Get the DOWS mintableSupply for week 2
			...
			const xBTCAmount = await exchangeRates.effectiveValue(xUSD, tenK, xBTC);
			const xBTCAmountMinusFees = await feePool.amountReceivedFromExchange(xBTCAmount);
			await shadows.exchange(xBTC, xBTCAmountMinusFees, xUSD, { from: account1 });
			await shadows.exchange(xBTC, xBTCAmountMinusFees, xUSD, { from: account2 });
			...
			// // Account 1 exchanges all xBTC back to xUSD
			// const acc1xBTCBalance = await xBTCContract.balanceOf(account1, { from: account1 });
			// await shadows.exchange(xBTC, acc1xBTCBalance, xUSD, { from: account1 });
			// const amountAfterExchange = await feePool.amountReceivedFromExchange(acc1xBTCBalance);
			...
			// await shadows.burnSynths(amountAfterExchangeInUSD, { from: account1 });

			// // Get the DOWS mintableSupply for week 3
			...
			// await shadows.issueSynths(twentyK, { from: account1 });

			// // Get the DOWS mintableSupply for week 4
		...
		it('(Inverse) Issue xBTC then shift rate down 50% then calc rewards');
	});

	describe('3 Accounts issue 10K xUSD each in week 1', async () => {
		beforeEach(async () => {
			await shadows.issueSynths(tenK, { from: account1 });
			await shadows.issueSynths(tenK, { from: account2 });
			await shadows.issueSynths(tenK, { from: account3 });
		});

		it('Acc1 issues and burns multiple times and should have accounts 1,2,3 rewards 50%,25%,25%', async () => {
			// Acc 1 Issues 20K xUSD
			await shadows.issueSynths(tenK, { from: account1 });
			...
			// Acc1 Burns all
			await shadows.burnSynths(twentyK, { from: account1 });
			// Acc 1 Issues 10K xUSD
			await shadows.issueSynths(tenK, { from: account1 });
			// Acc 1 Issues 10K xUSD again
			await shadows.issueSynths(tenK, { from: account1 });
	...
	describe('Collateralisation Ratio Penalties', async () => {
		...
		it('should apply no penalty when users claim rewards above the penalty threshold ratio of 1%', async () => {
			// Decrease DOWS collateral price by .9%
			const currentRate = await exchangeRates.rateForCurrency(DOWS);
			const newRate = currentRate.sub(multiplyDecimal(currentRate, toUnit('0.009')));

			const timestamp = await currentTime();
			await exchangeRates.updateRates([DOWS], [newRate], timestamp, {
				from: oracle,
			});
			...
			const dowsRewards = await feePool.feesAvailable(account1);
			assert.bnClose(dowsRewards[1], third(periodOneMintableSupplyMinusMinterReward));
		...
		it('should block user from claiming fees and rewards when users claim rewards >10% threshold collateralisation ratio', async () => {
			// But if the price of DOWS decreases a lot...
			const newRate = (await exchangeRates.rateForCurrency(DOWS)).sub(toUnit('0.09'));
			const timestamp = await currentTime();
			await exchangeRates.updateRates([DOWS], [newRate], timestamp, {
				from: oracle,
			});
