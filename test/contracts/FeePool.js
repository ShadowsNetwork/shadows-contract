...
contract('FeePool', async accounts => {
	...
	it('should set constructor params on deployment', async () => {
		...
		// constructor(address _proxy, address _owner, Shadows _shadows, FeePoolState _feePoolState, FeePoolEternalStorage _feePoolEternalStorage, IShadowsState _shadowsState, IShadowsEscrow _rewardEscrow, uint _exchangeFeeRate)
	...
	it('should correctly roll over unclaimed fees when closing fee periods', async () => {
		...
		await xUSDContract.transfer(account1, toUnit('10000'), {
			from: owner,
		});

		// Assert that the correct fee is in the fee pool.
		const fee = await xUSDContract.balanceOf(FEE_ADDRESS);
	...
	it('should allow a user to claim their fees in xUSD', async () => {
		const length = (await feePool.FEE_PERIOD_LENGTH()).toNumber();

		// Issue 10,000 xUSD for two different accounts.
		await shadows.transfer(account1, toUnit('1000000'), {
			from: owner,
		});

		await shadows.issueSynths(toUnit('10000'), { from: owner });
		await shadows.issueSynths(toUnit('10000'), { from: account1 });
		...
		for (let i = 0; i <= length; i++) {
			...
			await shadows.exchange(xUSD, exchange1, xAUD, { from: owner });
			await shadows.exchange(xUSD, exchange2, xAUD, { from: account1 });
		...
		const oldxUSDBalance = await xUSDContract.balanceOf(owner);
		...
		assert.eventEqual(claimFeesTx, 'FeesClaimed', {
			xUSDAmount: feesAvailableUSD[0],
			dowsRewards: feesAvailableUSD[1],
		});

		const newUSDBalance = await xUSDContract.balanceOf(owner);
		// We should have our fees
		assert.bnEqual(newUSDBalance, oldxUSDBalance.add(feesAvailableUSD[0]));
	});
	...
	it('should allow a user to claim their fees if they minted debt during period', async () => {
		...
		const oldSynthBalance = await xUSDContract.balanceOf(owner);

		// Now we should be able to claim them.
		await feePool.claimFees({ from: owner });

		// We should have our fees
		assert.bnEqual(await xUSDContract.balanceOf(owner), oldSynthBalance.add(feesAvailable[0]));
	...
	it('should allow a user to claim their fees in xUSD (as half of total) after some exchanging', async () => {
		const length = (await feePool.FEE_PERIOD_LENGTH()).toNumber();
	...
	it('should track fee withdrawals correctly', async () => {
		...
		// Issue xUSD for two different accounts.
		await shadows.transfer(account1, toUnit('1000000'), {
			from: owner,
		});
		...
		await shadows.exchange(xUSD, exchange, xAUD, { from: owner });
	...
	it('should correctly calculate the totalFeesAvailable for a single open period', async () => {
		...
		await shadows.issueSynths(amount, { from: owner });
		await shadows.issueSynths(amount.mul(web3.utils.toBN('2')), { from: account1 });

		// Generate a fee.
		await shadows.exchange(xUSD, amount, xAUD, { from: owner });
	...
	it('should correctly calculate the totalFeesAvailable for multiple periods', async () => {
		...
		await shadows.issueSynths(amount1, { from: owner });
		await shadows.issueSynths(amount2, { from: account1 });

		// Generate a fee.
		await shadows.exchange(xUSD, amount1, xAUD, { from: owner });
		...
		// Generate a fee.
		await shadows.exchange(xUSD, amount2, xAUD, { from: account1 });
	...
	describe('FeeClaimablePenaltyThreshold', async () => {
		...
		it('should be no penalty if issuance ratio is less than target ratio', async () => {
			...
			const newRate = (await exchangeRates.rateForCurrency(DOWS)).add(web3.utils.toBN('1'));
			const timestamp = await currentTime();
		...
		it('should correctly calculate the 10% buffer for penalties at specific issuance ratios', async () => {
			...
			// Increase the price so we start well and truly within our 20% ratio.
			const newRate = (await exchangeRates.rateForCurrency(DOWS)).add(
			...
			const issuanceRatio = fromUnit(await shadowsState.issuanceRatio());
			...
			// Start from the current price of shadows and slowly decrease the price until
			// we hit almost zero. Assert the correct penalty at each point.
			while ((await exchangeRates.rateForCurrency(DOWS)).gt(step.mul(web3.utils.toBN('2')))) {
				const ratio = await shadows.collateralisationRatio(owner);
				...
				// Bump the rate down.
				const newRate = (await exchangeRates.rateForCurrency(DOWS)).sub(step);
				const timestamp = await currentTime();
				await exchangeRates.updateRates([DOWS], [newRate], timestamp, {
					from: oracle,
				});
		...
		it('should revert when users try to claim fees with > 10% of threshold', async () => {
			...
			const amount = await xUSDContract.balanceOf(account1);
			await shadows.issueSynths(amount, { from: owner });
			await closeFeePeriod();
			...
			// But if the price of DOWS decreases by 15%, we will lose all the fees.
	...
	describe('claimOnBehalf and approveClaimOnBehalf', async () => {
		async function generateFees() {
			...
			// generate fee
			await shadows.exchange(xUSD, exchange1, xAUD, { from: account1 });
		...
		it('should approve a claim on behalf for account1 by account2 and have fees in wallet', async () => {
			...
			const oldSynthBalance = await xUSDContract.balanceOf(account1);
			...
			assert.bnEqual(await xUSDContract.balanceOf(account1), oldSynthBalance.add(feesAvailable[0]));
		});
	...
	describe('reducing FEE_PERIOD_LENGTHS', async () => {
		it('should be able to get fees available when feePoolState issuanceData is 6 blocks', async () => {
			...
			for (let i = 0; i <= length * 2; i++) {
				...
				await shadows.issueSynths(toUnit('1000'), { from: owner });
				await shadows.issueSynths(toUnit('1000'), { from: account1 });
	...
	describe('Escrowing Tokens', async () => {
		...
		it('should escrow tokens on an address when called by owner', async () => {
			...
			await shadows.approve(feePool.address, escrowAmount, {
				from: owner,
			});
