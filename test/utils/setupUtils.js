...
module.exports = {
	...
	async getDecodedLogs({ hash }) {
		...
		const shadows = await Shadows.deployed();
		const synthContract = await Synth.at(await shadows.synths(toBytes32('xUSD')));

		// And required ABIs to fully decode them
		abiDecoder.addABI(shadows.abi);
	...
	async issueSynthsToUser({ owner, user, amount, synth }) {
		...
		const synthContract = await Synth.at(await shadows.synths(synth));

		// First override the resolver to make it seem the owner is the Shadows contract
		await addressResolver.importAddresses(['Shadows'].map(toBytes32), [owner], {
			from: owner,
		});
