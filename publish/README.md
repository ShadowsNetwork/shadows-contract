...
This script can `build` (compile and flatten), `deploy` and `verify` (on Etherscan) the Shadows code to a testnet or mainnet.
...
```bash
node publish nominate -n rinkeby -d publish/deployed/rinkeby -g 3 -c Shadows -c ProxyxUSD -o 0x0000000000000000000000000000000000000000
node publish nominate -o 0xB64fF7a4a33Acdf48d97dab0D764afD0F6176882 -n kovan -c ProxyxUSD -d publish/deployed/kovan -g 20
```
...
Will attempt to remove all given synths from the `Shadows` contract (as long as they have `totalSupply` of `0`) and update the `config.json` and `synths.json` for the deployment folder.
...
```bash
node publish remove-synths -n rinkeby -d publish/deployed/rinkeby -g 3 -s sRUB -s xETH
```
...
Th `generate-token-list` command will generate an array of token proxy addresses for the given deployment to be used in the Shadows website. The command outputs a JSON array to the console.
