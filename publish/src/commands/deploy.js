...
const DEFAULTS = {
	...
	contractDeploymentGasLimit: 6.9e6, // TODO split out into seperate limits for different contracts, Proxys, Synths, Shadows
	network: 'kovan',
...
const deploy = async ({
	...
	let currentShadowsSupply;
	let currentExchangeFee;
	let currentShadowsPrice;
	let oldExrates;
	let currentLastMintEvent;
	let currentWeekOfInflation;

	try {
		const oldShadows = getExistingContract({ contract: 'Shadows' });
		currentShadowsSupply = await oldShadows.methods.totalSupply().call();

		// inflationSupplyToDate = total supply - 100m
		const inflationSupplyToDate = w3utils
			.toBN(currentShadowsSupply)
		...
		if (network === 'local') {
			...
			console.error(
				red(
					'Cannot connect to existing Shadows contract. Please double check the deploymentPath is correct for the network allocated'
				)
			);
			process.exitCode = 1;
			return;
	...
	try {
		...
		currentShadowsPrice = await oldExrates.methods.rateForCurrency(toBytes32('DOWS')).call();
		if (!oracleExrates) {
	...
	parameterNotice({
		...
		'Shadows totalSupply': `${Math.round(w3utils.fromWei(currentShadowsSupply) / 1e6)}m`,
			...
			await confirmAction(
				yellow(
					`⚠⚠⚠ WARNING: This action will deploy the following contracts to ${network}:\n${Object.entries(
						...
						.join(', ')}` + `\nIt will also set proxy targets and add synths to Shadows.\n`
				) +
					gray('-'.repeat(50)) +
	...
	const exchangeRates = await deployContract({
		name: 'ExchangeRates',
		args: [account, oracleExrates, [toBytes32('DOWS')], [currentShadowsPrice]],
	});
	...
	const shadowsEscrow = await deployContract({
		name: 'ShadowsEscrow',
		args: [account, ZERO_ADDRESS],
	});

	const shadowsState = await deployContract({
		name: 'ShadowsState',
	...
	const rewardsDistribution = await deployContract({
		...
		args: [
			account, // owner
			ZERO_ADDRESS, // authority (shadows)
			ZERO_ADDRESS, // Shadows Proxy
	...
	const proxyShadows = await deployContract({
		name: 'ProxyShadows',
		source: 'Proxy',
		args: [account],
	});

	const tokenStateShadows = await deployContract({
		name: 'TokenStateShadows',
		source: 'TokenState',
		args: [account, account],
	});

	const shadows = await deployContract({
		name: 'Shadows',
		deps: ['ProxyShadows', 'TokenStateShadows', 'AddressResolver'],
		args: [
			addressOf(proxyShadows),
			addressOf(tokenStateShadows),
			account,
			currentShadowsSupply,
			resolverAddress,
		],
	});

	if (proxyShadows && shadows) {
		await runStep({
			contract: 'ProxyShadows',
			target: proxyShadows,
			read: 'target',
			expected: input => input === addressOf(shadows),
			write: 'setTarget',
			writeArg: addressOf(shadows),
		});
	}
	...
	if (tokenStateShadows && config['TokenStateShadows'].deploy) {
		const initialIssuance = w3utils.toWei('100000000');
		await runStep({
			contract: 'TokenStateShadows',
			target: tokenStateShadows,
			read: 'balanceOf',
			readArg: account,
	...
	if (shadowsState && issuer) {
		// The ShadowsState contract has Issuer as it's associated contract (after v2.19 refactor)
		await runStep({
			contract: 'ShadowsState',
			target: shadowsState,
			read: 'associatedContract',
	...
	if (shadowsEscrow) {
		await deployContract({
			name: 'EscrowChecker',
			deps: ['ShadowsEscrow'],
			args: [addressOf(shadowsEscrow)],
		});
	}

	if (rewardEscrow && shadows) {
		await runStep({
	...
	if (config['Shadows'].deploy || config['ShadowsEscrow'].deploy) {
		// Note: currently on mainnet ShadowsEscrow.methods.shadows() does NOT exist
		// it is "havven" and the ABI we have here is not sufficient
		if (network === 'mainnet') {
			appendOwnerAction({
				key: `ShadowsEscrow.setHavven(Shadows)`,
				target: addressOf(shadowsEscrow),
				action: `setHavven(${addressOf(shadows)})`,
			});
		} else {
			await runStep({
				contract: 'ShadowsEscrow',
				target: shadowsEscrow,
				read: 'shadows',
				expected: input => input === addressOf(shadows),
				write: 'setShadows',
				writeArg: addressOf(shadows),
	...
	if (supplySchedule && shadows) {
		await runStep({
			contract: 'SupplySchedule',
			target: supplySchedule,
			read: 'shadowsProxy',
			expected: input => input === addressOf(proxyShadows),
			write: 'setShadowsProxy',
			writeArg: addressOf(proxyShadows),
		});
	}

	// Setup Shadows and deploy proxyERC20 for use in Synths
	const proxyERC20Shadows = await deployContract({
		name: 'ProxyERC20',
		deps: ['Shadows'],
		args: [account],
	});

	if (shadows && proxyERC20Shadows) {
		await runStep({
			contract: 'ProxyERC20',
			target: proxyERC20Shadows,
			read: 'target',
			expected: input => input === addressOf(shadows),
			write: 'setTarget',
			writeArg: addressOf(shadows),
		...
		await runStep({
			...
			expected: input => input === addressOf(proxyERC20Shadows),
			write: 'setIntegrationProxy',
			writeArg: addressOf(proxyERC20Shadows),
		});
	}

	if (shadows && rewardsDistribution) {
		await runStep({
	...
	for (const { name: currencyKey, inverted, subclass, aggregator } of synths) {
		...
		// xUSD proxy is used by Kucoin and Bittrex thus requires proxy / integration proxy to be set
		const synthProxyIsLegacy = currencyKey === 'xUSD' && network !== 'local';
		...
		if (currencyKey === 'xETH') {
			proxyxETHAddress = addressOf(proxyForSynth);
		}

		let proxyERC20ForSynth;

		if (synthProxyIsLegacy) {
			// additionally deploy an ERC20 proxy for the synth if it's legacy (xUSD and not on local)
		...
		const synth = await deployContract({
			...
			deps: [`TokenState${currencyKey}`, `Proxy${currencyKey}`, 'Shadows', 'FeePool'],
			args: [
		...
		// Setup integration proxy (ProxyERC20) for Synth (Remove when xUSD Proxy cuts over)
		...
		// Now setup connection to the Synth with Shadows
		if (synth && shadows) {
			await runStep({
				contract: 'Shadows',
				target: shadows,
	...
	const depot = await deployContract({
		name: 'Depot',
		deps: ['ProxyShadows', 'SynthxUSD', 'FeePool'],
	...
	// ArbRewarder contract for xETH uniswap
	const arbRewarder = await deployContract({
		name: 'ArbRewarder',
		deps: ['Shadows', 'ExchangeRates'],
		args: [account],
	});
	...
	if (arbRewarder) {
		...
		// Ensure shadows ProxyERC20 on arbRewarder set
		await runStep({
			contract: 'ArbRewarder',
			target: arbRewarder,
			read: 'shadowsProxy',
		...
		// Ensure xETH uniswap exchange address on arbRewarder set
		const requiredUniswapExchange = '0xe9Cf7887b93150D4F2Da7dFc6D502B216438F244';
		const requiredSynthAddress = proxyxETHAddress;
		await runStep({
			contract: 'ArbRewarder',
			target: arbRewarder,
			read: 'uniswapAddress',
		...
		// Ensure xETH proxy address on arbRewarder set
		await runStep({
			contract: 'ArbRewarder',
			target: arbRewarder,
			read: 'synth',
		...
		await runStep({
			...
			writeArg: [
				...
				[
					...
					addressOf(shadows),
					addressOf(shadowsEscrow),
					addressOf(shadowsState),
					addressOf(deployer.deployedContracts['SynthxUSD']),
					addressOf(deployer.deployedContracts['SynthxETH']),
				],
			],
		});
	}
