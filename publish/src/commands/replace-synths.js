...
const replaceSynths = async ({
	...
	deployer.deployedContracts.SafeDecimalMath = {
		options: {
			address: dows.getTarget({ network, contract: 'SafeDecimalMath' }).address,
		},
	};
	...
	const resolverAddress = await Shadows.methods.resolver().call();
	...
	for (const { currencyKey, Synth, Proxy, TokenState } of deployedSynths) {
		...
		// 2. invoke Shadows.removeSynth(currencyKey) // owner
		await runStep({
			contract: 'Shadows',
			target: Shadows,
			read: 'synths',
		...
		// 4. Shadows.addSynth(newone) // owner
		await runStep({
			contract: 'Shadows',
			target: Shadows,
			read: 'synths',
