...
const removeSynths = async ({
		...
		try {
			await confirmAction(
				cyan(
					`${yellow(
						'⚠ WARNING'
					)}: This action will remove the following synths from the Shadows contract on ${network}:\n- ${synthsToRemove.join(
						'\n- '
					)}`
	...
	const { address: shadowsAddress, source } = deployment.targets['Shadows'];
	const { abi: shadowsABI } = deployment.sources[source];
	const Shadows = new web3.eth.Contract(shadowsABI, shadowsAddress);

	const shadowsOwner = await Shadows.methods.owner().call();
	...
	for (const currencyKey of synthsToRemove) {
		...
		const currentSynthInDOWS = await Shadows.methods.synths(toBytes32(currencyKey)).call();

		if (synthAddress !== currentSynthInDOWS) {
			console.error(
				red(
					`Synth address in Shadows for ${currencyKey} is different from what's deployed in Shadows to the local ${DEPLOYMENT_FILENAME} of ${network} \ndeployed: ${yellow(
						currentSynthInDOWS
		...
		// now check total supply (is required in Shadows.removeSynth)
		...
		if (shadowsOwner === account) {
			console.log(yellow(`Invoking Shadows.removeSynth(Synth${currencyKey})...`));
			await Shadows.methods.removeSynth(toBytes32(currencyKey)).send({
				from: account,
				gas: Number(gasLimit),
			...
			console.log(
				gray(
					`Removed ${currencyKey} from Shadows. Verify via ${etherscanLinkPrefix}/address/${shadowsAddress}#readContract`
				)
			);
		} else {
			appendOwnerAction({
				key: `Shadows.removeSynth(Synth${currencyKey})`,
				target: shadowsAddress,
